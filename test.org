...
(ert-deftest leaf~flatten-keywords ()
  (let ((keywords '(:c (a b c) :h d e f)))
    (alet (leaf~flatten-keywords keywords)
      it
      ;; (should (equal it '(:c a :c b :c c :h d :h e :h f)))
      )))
#+end_src

** leaf~general-process
:PROPERTIES:
:ID:       3b89a49a-30a7-4c3b-9f93-8c7b1056c2b9
:END:

#+begin_src emacs-lisp
(ert-deftest leaf~general-process ()
  (should (eq (leaf~general-process :a '(1 2 3 4))
              (list :a 1 2 3 4)))
  (should (eq (leaf~general-process :a '((1 2 3 4)))
              (list :a 1 2 3 4)))
  (should (eq (leaf~general-process :a (cons 1 2))
              (cons :a (cons 1 2))))
  (should (eq (leaf~general-process :a 1)
              (cons :a 1))))
#+end_src
...
#+begin_src emacs-lisp
(ert-deftest dp~~apply-setq ()
  (alet (dp~~apply-setq 'hello (cons 'a 3) nil)
...
#+begin_src emacs-lisp
(ert-deftest dp~forms ()
  (let ((dp~keywords (a-list :setq #'dp~~apply-setq))
        (alist (a-list :setq '(a . 3)))
        (package 'hello))
    (should (alist-get :setq alist))
    (should (equal (dp~apply-fn package nil (-first-item alist)) '((setq a 3))))
    ;; (-reduce-from (-partial #'dp~apply-fn package) nil alist)
    (should (equal (dp~forms 'hello alist) '((setq a 3))))))
#+end_src
...
#+begin_src emacs-lisp
(ert-deftest dp~normalize ()
  (alet (dp~normalize '(:foo a b c :fee b c :fii (hello)))
    (should (eq it (a-list :foo 'a :foo 'b :foo 'c :fee 'b :fee 'c))))

  (alet (dp~normalize '(:init (+ 1 1) (+ 2 2)))
...
#+begin_src emacs-lisp
(ert-deftest dp~true-key ()
  (let ((dp~alias-alist '((:ensure . :ho) (:ho . :laa) (:laa . :hee))))
    ;; return the real aliased key.
    (should (eq (dp~true-key :ensure) :hee))
    ;; return the original key when non aliases.
    (should (eq (dp~true-key :when) :when))))
#+end_src
...
#+begin_src emacs-lisp
(ert-deftest dp~forms ()
  (alet (dp~forms 'horse (dp~normalize '(:os mac :require t)))
    (should (equal it '((with-os! mac (require 'horse))))))
  (alet (dp~forms 'cow (dp~normalize ))))
#+end_src
