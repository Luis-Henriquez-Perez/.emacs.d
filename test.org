...
(ert-deftest def-package-set-weight ()
  (let ((def-package-weight-alist '((:foo 10)))
    ...
    (should (= (def-package-weight :foo) 15))
    (should (> (def-package-weight :zap)
               (def-package-weight :foo)))
    (alet (thread-last (-sort random-sort-fn key-alist)
            (-sort #'def-package-weight-sorter))
      (should (equal (map-keys it) '(:zap :foo :fop :fap))))))
#+end_src
...
(ert-deftest def-package-normalize-values ()
  (let ((def-package-normalizer-alist)
        (pair '(:foo a (b c (d e f)))))
    (def-package-set-normalizer :foo #'flatten-list)
    (should (-contains-p def-package-normalizer-alist (cons :foo #'flatten-list)))
    (should (equal (def-package-apply-normalize pair)
                   '(:foo a b c d e f)))))
#+end_src
...
(ert-deftest def-package-normalize-args ()
  (let ((list '(:setq-default (var . 1) :setq (var . 2))))
    (should (null (def-package-normalize-args list)))
    ))
#+end_src
...
(ert-deftest def-package-apply-fn ()
  (cl-letf* (((symbol-function 'apply-setq)
             ...
             (def-package-keywords (list (cons :setq #'apply-setq))))
...
(ert-deftest def-package-forms ()
  (alet (def-package-forms 'horse (def-package-normalize-args '(:os mac :require t)))
    (should (equal it '((with-os! mac (require 'horse))))))
  (alet (def-package-forms 'cow (def-package-normalize-args ))))
#+end_src
...
*** bind
:PROPERTIES:
:ID:       404f22d8-a367-4b6b-8ee3-994e456a9646
:END:

#+begin_src emacs-lisp
(ert-deftest def-package-bind-normalizer ()
  ())
#+end_src
