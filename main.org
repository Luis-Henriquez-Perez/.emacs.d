...
(after! feebleline
  (setq feebleline-msg-functions
        '((feebleline:mode-icon :fmt "%2s")
          ...
          (feebleline:saved-buffer-indicator :face font-lock-warning-face)
...
(defun feebleline:battery-status-indicator ()
  ...
  (let ((icon-fn #'all-the-icons:icon)
        (props '(:v-adjust 0.01))
        (charge (thread-last (funcall battery-status-function)
                  (battery-format "%p")
                  (string-to-number))))
    (alet (cond ((> charge 75) '(faicon "full"           "[####]"))
