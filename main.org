...
(defun use-package-handler/:trigger (name _keyword args rest state)
  (-let* (((where . advices) (car args)))
    (use-package-concat
     (use-package-process-keywords name rest state)
     (if ()
...
[[helpvar:VOID-MAIN-ORG-FILE][void-main-org-file]]. ~void-tangle-on-save-h~ is called whenever a buffer is being
...
#+begin_src emacs-lisp
(defun void/take-screenshot-current-window ()
  "Take a screenshot of the current Emacs window."
  (interactive)
  (-let* ((id (frame-parameter (selected-frame) 'window-id))
          ((x y width height) (window-pixel-edges))
          (geometry (format "%dx%d+%d+%d" width height x y))
          (filename (format "%s%s~%s.png" VOID-SCREENSHOT-DIR "image" (s-replace "\s" "~" (ts-format)))))
    (void-screenshot)))
#+end_src
...
*** auto commit
:PROPERTIES:
:ID:       475c0383-543e-4381-82c0-6b5328068424
:END:
...
:PROPERTIES:
:ID:       79df7157-adfd-4b29-8091-d95c64524455
:END:

#+begin_src emacs-lisp
(defadvice! get-credentials-from-pass (:override gac-push)
  "Get my credentials from pass."
  (let ((default-directory (file-name-directory (buffer-file-name buffer)))
        (entry (password-store-parse-entry "github"))
        (username (password-store-get-field entry "username"))
        (password (password-store-get entry))
        (repo (git-repo <buffer>))
        (command (format "%s:%s@%s" username password repository)))
    (alet (start-process "git" "*git-auto-push*" "git" "push")
      (set-process-sentinel it 'gac-process-sentinel)
      (set-process-filter it 'gac-process-filter))))
#+end_src
