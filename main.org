...
This headline is about actually installing all of the packages I use.
...
#+begin_src emacs-lisp
(defun void-prop-string-to-list (string)
...
(defun void-org-property-lists (org-file)
  "Return a list of org properties from `org-file'."
  (let* ((recipes nil)
        ...
        (while (re-search-forward regexp nil t)
          (let ((match (match-string-no-properties 1)))
            (when (and match (string-match-p "PACKAGE" match))
              (push (void-prop-string-to-list match) recipes))))))
    recipes))
#+end_src
