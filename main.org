...
(defun void--use-void-log (orign-fn &rest args)
  (noflet ((message (&rest argz) (void-log argz)))
    (apply orign-fn args)))
#+end_src
...
(defun void-autoload (package fn)
  ...
  (fset name `(lambda (&rest args)
                ;; find the base advice.
                (-each (list (void-advice-true-name name) ())
                  #'fmakunbound)
                ;; find all functions aliased.
                (fmakunbound #',name)
                (require package)
...
***** make idle require use void-log                                  :disabled:
:PROPERTIES:
...
#+begin_src emacs-lisp
(--each '(transpose-frame flip-frame flop-frame rotate-frame rotate-frame-clockwise)
