...
#+begin_src emacs-lisp
(use-package:insert :trigger :commands)

(defun use-package-normalize/:trigger (name keyword args)
  (use-package:args-or-error keyword args #'symbolp "a symbol"))

(defun use-package-handler/:trigger (name _keyword dependencies rest state)
  (use-package-concat
   (use-package-process-keywords name rest state)
   (list
    (if-let ((commands (plist-get rest :commands))
             (advice (intern (format "load-dependencies-for-%s" name)))
             (body (--map `(require ',it) dependencies)))
        `(defadvice! ,advice (:before ,@commands)
           (void-log "Loading needed libraries for %s." ',name)
           ,@body)
      `(after! ,name
         (void-log "Loading needed libraries for %s." ',name)
         ,@body)))))
...
The purpose of this keyword. This is similar to an existing keyword that is
provided with =use-package= called =:=. The reason I don't use it is because that
keyword is too coarse: it perfoms this check with an =eval-after-load= form.

#+begin_src emacs-lisp
(use-package:insert :system-ensure :commands)

(defun use-package-normalize/:system-ensure (name keyword args)
  (use-package:args-or-error keyword args #'symbolp "a symbol"))

(defun use-package-handler/:system-ensure (name _keyword packages rest state)
  (use-package-concat
   (use-package-process-keywords name rest state)
   (list
    (when-let* ((commands (plist-get rest :commands))
                (advice (intern (format "ensure-system-packages-for-%s" name)))
                (packages (-map #'symbol-name packages)))
      `(defadvice! ,advice (:around ,@commands)
         (require 'system-packages)
         (if-let ((uninstalled (-select (-not #'system-packages-package-installed-p) ',packages))
                  (prompt (format "%S is needed for %s to work.\nWould you like to install it?" ',packages ',name)))
             (if (y-or-n-p prompt)
                 (progn (let (system-packages-noconfirm)
                          (system-packages-install ,(string-join packages "\s")))
                        (apply <orig-fn> <args>))
               (message "Aborting the loading of %s" ',name))
           (apply <orig-fn> <args>)))))))
#+end_src
...
*** defcommand!
:PROPERTIES:
:ID:       52363ccb-c9f7-4428-882d-ec2ee120057a
:END:

**** defcommand!
:PROPERTIES:
:ID:       fd5db6dc-9647-45d9-be02-03e2a5cd5564
:END:
...
**** font lock                                                        :disabled:
:PROPERTIES:
:ID:       b1f14c3a-bb91-4fa1-ae66-922db2b74d6b
:END:

#+begin_src emacs-lisp
(defhook! enable-defcommand-font-lock ()
  "Enable font-lock for `defcommand!'."
  )
#+end_src
...
**** calc
:PROPERTIES:
:ID:       a9617132-5112-4099-bc99-3abee4919bc7
:END:
...
***** system-packages
:PROPERTIES:
:ID:       55a3368b-34e7-495a-86f9-3386ed90038c
:END:

System packages provides an abstraction for operating on packages across
different systems. Considering that some emacs packages require.

#+begin_src emacs-lisp
(use-package! system-packages
  :popup ("\\*system-packages"
          (display-buffer-at-bottom)
          (window-height . 0.5))
...
** perspective
:PROPERTIES:
:ID:       f646341e-018a-44f7-9907-bd9dc964ae99
:END:

#+begin_src emacs-lisp
(use-package! perspective
  :setq (persp-state-default-file . (concat VOID-DATA-DIR "persp-state")))
#+end_src
...
*** selectrum
:PROPERTIES:
:ID:       962a027b-ed50-4bf8-8def-a00a199517bf
:END:
...
#+begin_src emacs-lisp
(after! selectrum
  (defadvice! dont-sort-with-selectrum (:around void/goto-line void/set-font void/switch-buffer)
...
**** orderless
:PROPERTIES:
:ID:       2c707079-0dfe-4909-81af-0d5fa215cff4
:END:

#+begin_src emacs-lisp
(use-package! orderless
  :before-call (orderless-filter orderless-highlight-matches)
  :init
  (after! selectrum
...
(use-package! engine-mode
  :straight (:host github :branch "main" :repo "hrs/engine-mode")
  :config
  (defengine amazon
...
** browse-url
:PROPERTIES:
:ID:       9eda7617-8492-4459-87db-ba5d92222a9b
:END:
...
** search web
:PROPERTIES:
:ID:       b96fbbf4-5092-4f4c-a9bb-e59f6ede4f98
:END:

=web-search= provides an aptly named command "web-search" to browse the web with.
=browse-url= is nice, but I never know the exact url I want to visit.

#+begin_src emacs-lisp
(use-package! search-web
  :commands search-web
  :init
  (defun search-web/default-engine ()
    (interactive)
    (search-web "duck" (read-string "Search: "))))
#+end_src
...
*** escr
:PROPERTIES:
:ID:       f71ee7f4-752d-4d90-9cf9-874cb1a1af11
:END:

#+begin_src emacs-lisp
(use-package! escr
  :straight (:host github :repo "atykhonov/escr")
  :commands (escr-window-screenshot escr-frame-screenshot escr-window-screenshot)
  :setq
  (escr-screenshot-quality . 10)
  (escr-screenshot-directory . VOID-SCREENSHOT-DIR))
#+end_src
...
(after! feebleline
  (setq feebleline-msg-functions
        '(
          ...
          ;; (feebleline:saved-buffer-icon :face font-lock-warning-face)
          (feebleline-file-modified-star :face font-lock-warning-face)
          (feebleline:git-branch-icon)
          (feebleline-git-branch :face feebleline-git-face)
          (feebleline:current-track)
          (feebleline:battery :align right)
...
(defun all-the-icons:icon (type name &rest properties)
  ...
  (if (void-icons-displayable-p)
      (apply (void-symbol-intern 'all-the-icons- type) properties)
...
(defun feebleline:git-branch-icon ()
  (when (and (void-icons-displayable-p)
...
*** time
:PROPERTIES:
:ID:       c2e0ce83-e2e4-4930-99f8-3c1eb53fdd56
:END:
...
*** saved buffer
:PROPERTIES:
:ID:       d5b65ce4-8947-44ba-b424-458ec8a196b4
:END:
...
#+begin_src emacs-lisp
(defun feebleline:saved-buffer-indicator ()
  "Display saved icon if buffer has been saved."
  (when (and (buffer-file-name) (buffer-modified-p))
    (all-the-icons-material "save" :v-adjust -0.01)))
#+end_src

**** saved status
:PROPERTIES:
:ID:       eba2ad0e-11de-4c2f-9748-2a4d699886c3
:END:

#+begin_src emacs-lisp
(defun feebleline:saved-buffer-status ()
  ""
  )
#+end_src
...
**** battery
:PROPERTIES:
:ID:       af0ce658-56a8-4b53-b707-210432e8dda4
:END:

#+begin_src emacs-lisp
(defun feebleline:battery ()
  "Return the battery percentage display of feebleline."
  (require 'battery)
  (let* ((battery-fn (-rpartial #'battery-format (funcall battery-status-function)))
         (percent (string-to-number (funcall battery-fn "%p")))
         (icon-fn #'all-the-icons-faicon))
    (--> (cond ((string= (funcall battery-fn "%B") "Charging")
                (setq icon-fn #'all-the-icons-alltheicon)
                "-charging")
               ((> percent 75) "-full")
               ((> percent 50) "-three-quarters")
               ((> percent 25) "-half")
               ((> percent 10) "-quarter")
               (t "-empty"))
         (funcall icon-fn (concat "battery" it) :v-adjust 0.01)
         (format "%s %%%d" it percent))))
#+end_src
...
#+begin_src emacs-lisp
(defun feebleline:battery-status ()
  "docstring"
  )
#+end_src
...
#+begin_src emacs-lisp
(defun feebleline:battery-percentage ()
  "Return battery percentage."
  (concat "%" (battery-format "%p" (funcall battery-status-function))))
#+end_src
...
#+begin_src emacs-lisp
(defun feebleline:track-status-indicator ()
  (when (bound-and-true-p emms-player-playing-p)
    (cond (emms-player-paused-p '(material "pause" :v-adjust 0.01 :fallback "*paused*"))
          (emms-repeat-track '(material "repeat_one" :fallback "*repeat-one*"))
          (t '(material "music_note" :fallback "*playing*")))))
#+end_src
