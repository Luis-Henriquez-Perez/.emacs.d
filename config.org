...
(defvar xl-load-path-database
  (db-make `(db-hash :from-filename ,(xl-data-dir "db/load-path"))))
#+end_src

*** ensuring a package
:PROPERTIES:
:ID:       3033c4ea-9f43-4aac-a11d-f3af3ccc655c
:END:

I want startup to be fast even without having to compile my elisp file. One way
to achieve this is to cache package load-paths.

#+begin_src emacs-lisp
(defun xl-ensure-package (package)
  "Make sure the package is installed.
First check if a package's load-path is cached. If it is, add its load-paths to
the thing."
  (acond ((db-get package xl-load-path-database)
	  (dolist (package) (cl-pushnew it load-path)))
	 ((alist-get package (xl-get-create-recipe-list))
	  (let ((load-path xl-base-load-path)
		(pre-install-load-path xl-base-load-path))
	    (straight-use-package it)
	    (-difference pre-install-load-path load-path))
	  (db-put package (list new-paths) x-package-database))
	 (t
	  (straight-use-package package))))
#+end_src
