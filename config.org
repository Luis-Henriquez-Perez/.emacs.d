...
#+begin_src emacs-lisp
(defvar xl-pre-install-packages '(dash loopy anaphora db)
...
(mapc #'require xl-pre-install-packages)
#+end_src
...
(defun xl--keyword-macro-args (body)
  ...
  (loopy (with (docstring (when (stringp (car body)) (pop body))))
	 ((cons (key value . rest) body #'cddr)
	  (unless (keywordp key)
	    (return (list docstring plist `(,key ,value ,@rest))))
	  (append plist (list key value)))
	 (else-do (return (list docstring plist rest)))))
#+end_src
...
#+begin_src emacs-lisp
(defun xl-hookify-symbol (symbol)
...
#+begin_src emacs-lisp
(defun xl-modeify-symbol (symbol)
  "Return SYMBOL converted to hook.
If SYMBOL is already a hook symbol, return it unchanged."
...
(config! use-package
  :init
  (cl-callf push use-package-defaults '(:xl-ensure t t))
...
(defun uuse-package-insert-keyword (where place key)
  (--splice (eq place it)
	    (funcall (if (eq where 'after) #'identity #'reverse) (list key it))
...
**** helper: list or list of one
:PROPERTIES:
:ID:       bef70822-b8d2-4496-9195-7003140fa4b3
:END:

Using =use-package= it is often the case that an argument could either be a list
with one element--a list containing a list of the actual values you're
interested in. Or, a list of the actual values you're interested in.

#+begin_src emacs-lisp
(defun uuse-package-list-or-one (list &optional fn)
  "Determine whether."
  (cond ((-all-p fn list) list)
	((and (xl-one-p (length list))
	      (-all-p fn (car-safe list))))
	(car list)
	(t nil)))
#+end_src
...
(uuse-package-defnormalize! xl-hook ()
  ;; get the cons cells
  ;; elements should be of the type.
  ;; symbol
  (->> <args>
       (--map-when (symbolp it) (cons it (xl-modeify-symbol <name>)))
       (-map #'-cons-to-list)
       (--map-when (-any-p #'listp it) (-map #'list it))
       (--splice (-any-p #'listp it) (apply #'-table-flat #'list))
       (--map-when (xl-mode-symbol-p (car it)) (xl-hookify-symbol it))))

(uuse-package-defhander! xl-hook ()
  "Expect an alist of (HOOK FN)."
  `(,@(-map (-lambda ((hook fn)) `(xl-add-hook ',hook ',fn)) <value>)
    ,@(--map (uuse-package-autoload <name> it)
	     (-map #'cdr <value>))
    ,@<body>))
#+end_src
...
(uuse-package-defnormalize! xl-set ()
  (aprog1 (if (and (= 1 (length <args>))
		   (xl-true-list-p (car <args>)))
	      (car <args>)
	    <args>)
    (cl-assert (and (xl-true-list-p it)
		    (-all-p #'listp it)))))
...
***** grugru
:PROPERTIES:
:ID:       87e3911d-e5c0-4cd0-8da9-525b651cf78f
:END:

#+begin_src emacs-lisp
(setq use-package-keywords
      (uuse-package-insert-keyword 'before :hook :xl-grugru))

(uuse-package-defnormalize! xl-grugru ()
  <args>)

(uuse-package-defhander! xl-grugru ()
  `(,@(--map <value>)
    ,@<body>))
#+end_src
...
***** advice
:PROPERTIES:
:ID:       0c3c0715-24d3-4cb9-8c9f-a5d29a4bdb57
:END:

***** file or directory
:PROPERTIES:
:ID:       cb408061-2c17-46dd-a286-a76a2907285f
:END:

I want to keep track of all the.

#+begin_src emacs-lisp
(defvar xl-package-files-and-directories nil
  "An alist.")
#+end_src
...
#+begin_src emacs-lisp
(config! dash-functional :demand t)
#+end_src
...
#+begin_src emacs-lisp
(db-make `(db-hash :filename ,(concat db:dir "load-path")))
#+end_src
...
**** defmacro!
:PROPERTIES:
:ID:       985ed5d5-a4fd-49ad-a61e-9d6d8e09d65d
:END:
...
**** defun!
:PROPERTIES:
:ID:       abcffa37-08f6-4ec4-b98e-d9b394344099
:END:
...
(config! files
  ...
  (setq backup-directory-alist (list (cons ".*" (xl-data-dir "backup/"))))
...
#+begin_src emacs-lisp
(after! server
  (setq server-auth-dir (xl-data-dir "server/")))
#+end_src
...
#+begin_src emacs-lisp
(setq nsm-settings-file (xl-data-dir "network-settings.data"))
#+end_src
...
(config! idle-require
  ...
  :xl-pause-gc idle-require-load-next
  :xl-set!
...
#+begin_src emacs-lisp
(config! company-prescient :hook company-mode-hook)
#+end_src
...
#+begin_src emacs-lisp
(defmacro! ttempo-snippet! (name args docstring &rest body)
  "Define a snippet."
  (declare (indent defun))
  (-let* ((name (symbol-name name))
    ...
    `(after! tempo
       (defvar tempo-<mode>+ "")
       (defvar ,taglist nil "Taglist for <mode>.")
       (tempo-define-template ,name ,body ,tag ,docstring ',taglist))))
#+end_src
...
(config! recentf
  ...
  (setq recentf-save-file (concat XL-DATA-DIR "recentf"))
...
#+begin_src emacs-lisp
(config! separedit :commands separedit)
#+end_src
...
#+begin_src emacs-lisp
(config! aggressive-fill-paragraph)
#+end_src
...
#+begin_src emacs-lisp
(config! aggressive-indent-mode :hook emacs-lisp-mode-hook)
#+end_src
...
#+begin_src emacs-lisp
(config! smartparens
  :load-before-call evil-expression evil-ex
...
#+begin_src emacs-lisp
(config! outshine :commands outshine-mode)
#+end_src
...
#+begin_src emacs-lisp
(config! evil-visualstart :config
  :general
...
#+begin_src emacs-lisp
(config! lispyville :hook emacs-lisp-mode-hook)
#+end_src
...
(config! macrostep
  :commands
  (macrostep-expand macrostep-collapse macrostep-collapse-all)
  :general
  (:infix "m"
...
#+begin_src emacs-lisp
(config! org-link-minor-mode :xl-hook outshine-mode-hook)
#+end_src
...
#+begin_src emacs-lisp
(config! org-capture
  :before-call (org-capture . (ts org-ml))
...
#+begin_src emacs-lisp
(config! org-clock
  :commands (list #'org-clock-in #'org-clock-goto))
...
*** org-element
:PROPERTIES:
:ID:       17876234-a5cd-4373-9a01-033c58b26ec5
:END:

#+begin_src emacs-lisp
(nameless-add-global-alias+ "org-element" "oe")
#+end_src
...
*** setup
:PROPERTIES:
:ID:       497be1b0-0cbf-46df-9dd8-f54363a8ae21
:END:
...
(config! dashboard
  ...
  (initial-buffer-choice #'xl-initial-buffer))
#+end_src
