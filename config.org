...
(use-package:defhander! xl-idle-require ()
  `((use-package! idle-require :config
  ...
  (-let* ((name (symbol-name name))
    ...
    `(after! tempo
	 ...
	 ,(format "List of `tempo' tags for `%s'" mode))
       (defhook! declare-tempo-tags (,(xl-hookify mode))
	 ,(format "Declare taglist to be used for `%s'." mode)
...
(use-package! mmt
  ;; :xl-alias
  ;; (once-only! . mmt-once-only)
  ;; (with-gensyms! . mmt-with-gensyms)
  ;; (with-gensyms! . mmt-with-gensyms)
  )
#+end_src
...
#+begin_src emacs-lisp
(defhook! enable-org-tempo-templates (org-desc:edit-mode-hook)
  (tempo-use-tag-list 'tempo:org-mode-taglist))

(xl-add-hook 'org-desc:edit-mode-hook #'company-mode)

;; Add an element to the company alist.
(after! company
  (push '(org-desc:edit-mode :only (company-tempo)) company:backend-alist))
#+end_src
...
#+begin_src emacs-lisp
(defvar org-desc:default-post-blanks 1
...
#+begin_src emacs-lisp
(defun org-desc:goto-last-non-white ()
  (goto-char (point-max))
  (skip-chars-backward "\n\s"))
#+end_src
...
#+begin_src emacs-lisp
(defun org-desc:add-newlines ()
  "Add newlines."
  (save-excursion
    (--dotimes (1+ org-desc:default-post-blanks)
      (insert "\n"))))
#+end_src
...
#+begin_src emacs-lisp
(define-minor-mode org-desc:edit-mode
...
(defun org/dwim-edit-description ()
  ...
  (-let* (((beg . end) (org-desc:bounds))
    ...
    (let ((edit-indirect-after-creation-hook edit-indirect-after-creation-hook)
      ...
      (when was-empty-p
	(add-hook 'edit-indirect-after-creation-hook #'org-desc:add-newlines 0))
      (add-hook 'edit-indirect-after-creation-hook #'org-desc:goto-last-non-white 0)
      (with-current-buffer (edit-indirect-region beg end)
	(rename-buffer (format "*Org Desc - %s*" headline) t)
	(org-desc:edit-mode 1)
	(add-hook 'edit-indirect-before-commit-hook
		  #'org-desc:maybe-remove-newline nil t)
...
#+begin_src emacs-lisp
(after! evil
  (xl-add-hook 'org-desc:edit-mode-hook #'evil-insert-state))
#+end_src
...
(alet (list #'auto-capitalize-mode #'aggressive-fill-paragraph-mode
	    #'org-link-minor-mode)
  (xl-add-hook 'org-desc:edit-mode-hook it))
#+end_src
