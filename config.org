...
(defhook! tangle-on-quit-maybe (kill-emacs-hook :append t)
  "Tangle `VOID-MAIN-ORG-FILE' if necessary."
...
The idea is I want to use one.

*** init
:PROPERTIES:
:ID:       e3ac93b2-889b-48e7-8458-d8935aece221
:END:

#+begin_src emacs-lisp
(dbc-add-rule "bottom" "edit-indirect" :newname "\\*edit-indirect\\*")
#+end_src

*** extend edit-indirect so I can customize the buffer name
:PROPERTIES:
:ID:       157e83c6-b25e-4d70-bcf1-63bbe6fd42f5
:END:
...
**** name for an org description
:PROPERTIES:
:ID:       f47f5e44-cca9-4bb7-be2f-471aa54bfaac
:END:

#+begin_src emacs-lisp
(dbc-add-rule "bottom" "org description" :newname "\\*edit-indirect\\*")
#+end_src
...
=edit-indirect= requires the beginning and end of the editing region, so I need to
determine the beginning and end of the description.

It is important to note that my tendency in org mode is to have a headline with
at most one description and at most one source block.
...
If there is no existing description return nil."
  (interactive)
  (let ((nodes (thread-first (org-ml-parse-subtree-at (point))
		 (org-element-map org-element-all-elements #'identity))))
    (aif (thread-last nodes
	   (--split-when (not (eq (org-element-type it) 'paragraph)))
	   (car))
	;; Get the set of paragraphs that are.
	(cons (org-element-property :begin (-first-item it))
	      (org-element-property :end (-last-item it)))
      ;; Assume a property drawer exists.
      (alet (thread-last nodes 
	      (--split-when (eq (org-element-type it) 'headline))
	      (car)
	      (--first (eq (org-element-type it) 'property-drawer)))
	(alet (org-element-property :end it)
	  (cons it it))))))
#+end_src
...
:PROPERTIES:
:ID:       9dde4377-ec03-462c-b3bd-5be8b98cc781
:END:

Creating a new description is trickier. One reason is because if I want newlines
separating a description from it's source block--and I do--then I need to
somehow add a newline as if it was already there. Also, I have to consider the
case I end up aborting the description or committing an empty description in
which case, then, I need to make sure not to add the newline.

***** entering a description
:PROPERTIES:
:ID:       0e2abde6-6bf0-4d5c-81fb-98874694e635
:END:

When we enter a description and there is no existing description, we want to
insert a blank like one that would be there if there were an existing
description.
...
**** create a hook for editing descriptions
:PROPERTIES:
...
(defun org/dwim-edit-description ()
  ...
  (-let [(beg . end) (org:description-bounds)]
    ...
    ;; We need to respect what could be in edit-indirect hooks.
...
Because org-mode already has lots of support for source blocks, they're way easier.
...
#+begin_src emacs-lisp
(void-load-before-call 'base16-theme (list #'load-theme #'consult-theme))
#+end_src
