...
** annotate
:PROPERTIES:
:ID:       704ee343-2b90-442f-8ce6-cb158c15b8d7
:TYPE:     git
:FLAVOR:   melpa
:HOST:     github
:REPO:     "bastibe/annotate.el"
:PACKAGE:  "annotate"
:LOCAL-REPO: "annotate.el"
:END:

This package allows you to add annotations to a file without actually editing
it. The annotations are stored in their own file, so they are persistent.

I prefer keeping code separate from documentation. This means I don't use inline
comments in code. Instead, I use these annotations. At least, this is the idea
in theory. As of writing this I've just added this package to my configuration
so I'm not sure whether I will find it useful.

*** init
:PROPERTIES:
:ID:       b52b2c19-ee6e-411a-917c-1b7b6887d11b
:END:

#+begin_src emacs-lisp
(setq annotate-file (concat VOID-DATA-DIR "annotations"))
#+end_src
...
*** helpers
:PROPERTIES:
:ID:       7a5d6767-c8fd-4b5c-8102-cc6ad919e7e6
:END:

**** equal shortcut
:PROPERTIES:
:ID:       47937704-e6e7-4900-abab-735c26b3cf36
:END:

#+begin_src emacs-lisp
(defun org-element:type-equal-p (element type)
  "Return non-nil if ELEMENT"
  (eq (org-element-type element) type))
#+end_src

**** elements under current heading
:PROPERTIES:
:ID:       6c82be4f-a775-4381-af7d-300eb8ff61b9
:END:
...
In the case where there was no existing description, we need to newlines to the
description buffer so that if we end up committing a change the description will
be placed properly in relation to what's below it. This hook is designed to make
sure this happens.
...
(defun org/dwim-edit-source-block ()
  ...
  (interactive)
  (-let* ((elements '(headline src-block))
	  (nodes (-> (org-ml-parse-subtree-at (point))
		     (org-element-map elements #'identity)))
	  (source-block (->> nodes
			     (--split-when (eq 'headline (org-element-type it)))
			     (car)
			     (--first (eq (org-element-type it) 'src-block))))
	  (beg (org-element-property :contents-begin source-block))
	  (end (org-element-property :contents-end source-block))
	  (headline (org-element-property :title-text (car nodes))))
    (unless source-block
      (user-error "No source block in headline."))
    (message "(%d , %d)" beg end)
    (with-current-buffer (edit-indirect-region beg end t)
      (rename-buffer (format "*Org Source - %S" headline)))
    ))
#+end_src
