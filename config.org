...
#+begin_src emacs-lisp
(defconst XL-EMACS-DIR (file-truename user-emacs-directory)
...
(defconst XL-DATA-DIR (concat XL-LOCAL-DIR "data/")
...
(defconst XL-ORG-DIR (concat XL-EMACS-DIR "org/")
...
#+begin_src emacs-lisp
(dolist (dir (list XL-LOCAL-DIR XL-DATA-DIR XL-PACKAGES-DIR))
...
#+begin_src emacs-lisp
(defvar xl-recipe-list (xl-package-recipes)
...
(mapc #'straight-use-package
      (progn (require 'map)
	     (map-delete (xl-package-recipes) 'straight)))
#+end_src
...
(defun xl-silence-output (fn)
  "Silence output for the duration of ORIGN-FN."
  (xl-add-advice fn :around #'xl--silence-output-advice))
#+end_src
...
(defun xl-pause-garbage-collection (fn)
  "Pause garbage collection for the duration of FN."
  (xl-add-advice fn :around #'xl--pause-garbage-collection-advice))
#+end_src
...
(defun xl-message-with-xl-log (fn)
  "Use `xl-log' instead of `message'."
  (xl-add-advice fn :around #'xl--message-with-xl-log-advice))
#+end_src
...
(setq initial-buffer-choice #'xl-initial-buffer)
#+end_src
...
(xl-add-hook 'prog-mode-hook #'show-paren-mode)
#+end_src
...
(setq backup-directory-alist (list (cons ".*" (concat XL-DATA-DIR "backup/"))))
...
#+begin_src emacs-lisp
(after! server
  (setq server-auth-dir (concat XL-DATA-DIR "server/")))
#+end_src
...
(after! tramp
  ...
  (setq tramp-auto-save-directory (concat XL-DATA-DIR "tramp-auto-save/"))
  (setq tramp-persistency-file-name (concat XL-DATA-DIR "tramp-persistency.el")))
#+end_src
...
#+begin_src emacs-lisp
(setq url-cache-directory (concat XL-DATA-DIR "url/cache/"))
(setq url-configuration-directory (concat XL-DATA-DIR "url/configuration/"))
#+end_src
...
(setq diary-file (concat XL-DATA-DIR "diary"))
...
#+begin_src emacs-lisp
(setq nsm-settings-file (concat XL-DATA-DIR "network-settings.data"))
#+end_src
...
#+begin_src emacs-lisp
(xl-add-advice #'display-startup-echo-area-message :override #'ignore)
#+end_src
...
(xl-add-advice 'startup-echo-area-message :override #'ignore)
#+end_src
...
(xl-add-hook 'emacs-startup-hook #'gcmh-mode t)
#+end_src
...
#+begin_src emacs-lisp
(xl-pause-garbage-collection #'load-theme)
#+end_src
...
#+begin_src emacs-lisp
(xl-add-hook 'emacs-startup-hook #'keyfreq-mode)
...
#+begin_src emacs-lisp
(xl-message-with-xl-log #'idle-require-mode)
(xl-message-with-xl-log #'idle-require-load-next)
#+end_src
...
#+begin_src emacs-lisp
(xl-pause-garbage-collection #'idle-require-load-next)
#+end_src
...
(--each (list #'consult-buffer #'consult-buffer-other-window)
  (xl-load-before-call 'recentf it t))
#+end_src
...
(xl-add-hook 'prog-mode-hook #'company-mode)
#+end_src
...
(xl-add-hook 'company-mode-hook #'company-prescient-mode)
#+end_src
...
#+begin_src emacs-lisp
(xl-add-hook 'emacs-startup-hook #'selectrum-mode)
...
#+begin_src emacs-lisp
(xl-add-hook 'selectrum-mode-hook #'prescient-persist-mode)

(setq prescient-save-file (concat XL-DATA-DIR "prescient-save-file"))
#+end_src
...
#+begin_src emacs-lisp
(xl-add-hook 'selectrum-mode-hook #'selectrum-prescient-mode)
...
#+begin_src emacs-lisp
(setq emms-directory (concat XL-DATA-DIR "emms/"))
...
#+begin_src emacs-lisp
(xl-add-hook 'mu4e-compose-mode-hook #'org-mu4e-compose-org-mode)
...
#+begin_src emacs-lisp
(setq eshell-directory-name (concat XL-DATA-DIR "eshell/"))
...
(setq transient-levels-file (concat XL-DATA-DIR "transient/levels"))
(setq transient-values-file (concat XL-DATA-DIR "transient/values"))
(setq transient-history-file (concat XL-DATA-DIR "transient/history"))
#+end_src
...
(ignore! (xl-add-hook 'magit-popup-mode-hook #'hide-mode-line-mode))
...
(xl-silence-output #'evil-magit-init)
...
#+begin_src emacs-lisp
(xl-load-after-call #'after-find-file #'saveplace t)

(setq save-place-file (concat XL-DATA-DIR "saveplace"))
...
(xl-load-before-call 'recentf #'find-file t)
#+end_src
...
(gsetq recentf-save-file (concat XL-DATA-DIR "recentf"))
...
#+begin_src emacs-lisp
(xl-add-advice #'recentf-save-list :before #'recentf-cleanup)
#+end_src
...
(setq savehist-file (concat XL-DATA-DIR "savehist"))

(idle-require 'custom)
(xl-add-hook 'emacs-startup-hook #'savehist-mode)
...
#+begin_src emacs-lisp
(xl-add-hook 'tab-bar-mode-hook #'tab-bar-history-mode)
#+end_src
...
(setq desktop-dirname XL-DATA-DIR)
(setq desktop-auto-save-timeout auto-save-timeout)
(setq desktop-base-file-name "emacs.desktop")
(setq desktop-base-lock-name "emacs.desktop.lock")
(setq desktop-path (list XL-DATA-DIR))
...
(setq wg-session-file (concat XL-DATA-DIR "wg-session"))
#+end_src
...
#+begin_src emacs-lisp
(xl-add-hook 'text-mode-hook #'auto-capitalize-mode)
...
#+begin_src emacs-lisp
(xl-load-before-call 'super-save #'find-file t)
...
(xl-add-hook 'emacs-lisp-mode-hook #'aggressive-indent-mode)
#+end_src
...
(alet '(prog-mode-hook eshell-mode-hook ielm-mode-hook)
  (xl-add-hook it #'smartparens-strict-mode))
#+end_src
...
#+begin_src emacs-lisp
(xl-load-before-call 'smartparens (list #'evil-expression #'evil-ex) t)
...
#+begin_src emacs-lisp
(xl-add-hook '(prog-mode-hook reb-mode-hook) #'rainbow-delimiters-mode)
...
(xl-add-hook '(prog-mode-hook text-mode-hook) #'evil-surround-mode)
#+end_src
...
(xl-add-hook 'window-setup-hook #'evil-mode)
#+end_src
...
#+begin_src emacs-lisp
(xl-silence-output #'keyboard-quit)
#+end_src
...
(alet (list #'evil-insert-state #'evil-emacs-state)
  (xl-load-before-call 'key-chord it t))
#+end_src
...
#+begin_src emacs-lisp
(xl-silence-output #'key-chord-mode)
#+end_src
...
(xl-add-hook 'emacs-lisp-mode-hook #'lispyville-mode)
#+end_src
...
(xl-add-hook 'emacs-lisp-mode-hook #'lispy-mode)
#+end_src
...
#+begin_src emacs-lisp
(defvar xl-elisp-tempo-tags nil
...
(xl-add-hook 'outshine-mode-hook #'org-link-minor-mode)
#+end_src
...
;; (-each '(ts org-ml) (-partial #'xl-load-before-call #'org-capture))
(--each '(ts org-ml)
  (xl-load-before-call it #'org-capture))
#+end_src
...
(setq org-clock-persist-file (concat XL-DATA-DIR "org-clock-save.el"))
(setq org-clock-in-resume t)

(after! org-clock
  (xl-add-hook 'kill-emacs-hook #'org-clock-save)
...
#+begin_src emacs-lisp
(setq org-id-locations-file (concat XL-DATA-DIR "org-id-locations"))
...
#+begin_src emacs-lisp
(setq org-id-files (list XL-MAIN-ORG-FILE XL-RECIPE-FILE))
#+end_src
...
#+begin_src emacs-lisp
(xl-add-hook 'org-insert-heading-hook #'org-id-get-create)
#+end_src
...
#+begin_src emacs-lisp
(xl-add-hook 'org-mode-hook #'org-superstar-mode)
...
(xl-add-hook 'emacs-startup-hook #'mini-modeline-mode)
#+end_src
...
#+begin_src emacs-lisp
(xl-add-hook 'emacs-startup-hook #'which-key-mode)
...
#+begin_src emacs-lisp
(require 'dashboard)
(xl-add-hook 'window-setup-hook #'dashboard-insert-startupify-lists)
#+end_src
...
#+begin_src emacs-lisp
(defadvice! open-dashboard-instead (:around xl-initial-buffer)
  (if xl-debug-p
      (apply <orig-fn> <args>)
...
(setq dashboard-startup-banner
      (seq-random-elt (if (display-graphic-p) '(official logo) '(1 2 3))))
...
(defadvice! set-custom-banner-path (:override dashboard-get-banner-path)
  ...
  (concat XL-LOCAL-DIR "xl-banner.txt"))
#+end_src
...
#+begin_src emacs-lisp
(xl-add-hook 'window-setup-hook #'window-divider-mode)
#+end_src
