...
(defun org/dwim-edit-source-block ()
  ...
  (alet (->> (org-ml-parse-subtree-at (point))
    ...
    (org-element-property :contents-begin source-block)
    (org-element-property :contents-end source-block)
    (let (())
      (with-current-buffer (edit-indirect-region beg end t)
	(rename-buffer (format "*Org Source - %S"))
	(when (= beg end)
	  )))))
#+end_src
...
A common annoyance is dealing accidentally.
