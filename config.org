...
*** reproducability
:PROPERTIES:
:ID:       e8871b11-d6bc-4577-bfa9-c8ae82867e1f
:END:

**** revision
:PROPERTIES:
:ID:       0f8d5286-2b4f-4f3f-a0a7-259c7d279171
:END:

This function is based on one provided by "" in [[][]].

#+begin_src emacs-lisp
(defun straight:revision (package)
  "Return the revision of PACKAGE."
  (straight--with-plist (straight:recipe package)
      (local-repo type)
...
**** recipe
:PROPERTIES:
:ID:       55f67139-1b77-4dc8-96e2-139149d540e3
:END:

#+begin_src emacs-lisp
(defun straight:recipe (package)
  "Return recipe for an existing package."
  (hash-get package straight--recipe-cache))
#+end_src

**** set repository to a specific revision
:PROPERTIES:
:ID:       f7fbb68b-ede5-4458-9a16-afec10aba6be
:END:

To write this function I looked at [[helpfn:straight-thaw-versions][straight-thaw-versions]].

#+begin_src emacs-lisp
(defun straight:set-package-to-revision (package revision)
  "Set package to specific revision."
  (unless (straight-vc-commit-present-p recipe revision)
    (straight-vc-fetch-from-remote recipe))
  (straight-vc-check-out-commit recipe revision))
#+end_src

**** write recipes and revisions
:PROPERTIES:
:ID:       ae2a4253-2164-4b96-9ed8-67e368318053
:END:

I want one file I can read to get the list of packages I need to install and any
information I need to install them. Separately, will be this file which contains
the description and configuration of those packages. =straight.el= offers a
lockfile but I need the recipes to be completely.

Note that I write everything as a plist because it's easier.

#+begin_src emacs-lisp
(defun straight:write-recipes ()
  "Write the information needed for all installed packages."
  (loopy ((list package (hash-table-keys straight--recipe-cache))
	  (expr recipe (straight:recipe package))
	  (expr revision (straight:revision package))
	  (collect recipe-list `(:revision ,revision ,@recipe)))
	 (finally-do
	  (with-temp-file (concat user-emacs-directory "recipes.el")
	    (insert (pp recipe-list))))))
#+end_src

**** load packages from recipes and revisions
:PROPERTIES:
:ID:       72e642aa-19c3-4df6-911d-ad8bed207058
:END:

#+begin_src emacs-lisp
(defun void--read-recipe-file ()
  "Return a list whose cons cells."
  ())
#+end_src
