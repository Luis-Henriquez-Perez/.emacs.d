    ...
    (while stack
      ...
      (pcase frame
        ...
        (`(nil . ((,(or 'nflet! 'noflet!) . ,args) . ,body))
         (push `(:shortcut lef! ,args) stack)
         (push `(nil . (progn ,@body)) stack))
        (`(nil . ((,(or 'flet! 'stub!) . ,args) . ,body))
         (push `(:shortcut cl-flet ,args) stack)
         (push `(nil . (progn ,@body)) stack))
        (`(nil . ((label! . ,args) . ,body))
         (push `(:shortcut cl-labels ,args) stack)
         (push `(nil . (progn ,@body)) stack))
        (`(nil . ((,(or 'mlet! 'macrolet!) . ,args) . ,body))
         (push `(:shortcut cl-macrolet ,args) stack)
         (push `(nil . (progn ,@body)) stack))
        (`(:shortcut ,type ,args)
