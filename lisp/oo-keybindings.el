...
;;;; main bindings
;;;;; leader prefix
(keymap-set i override-global-map oo-insert-leader-key #'oo-leader-prefix-command)
(keymap-set override-global-map oo-emacs-leader-key  #'oo-leader-prefix-command)
(keymap-set override-global-map oo-emacs-alt-leader-key  #'oo-leader-prefix-command)
(keymap-set (n m v) override-global-map oo-normal-leader-key #'oo-leader-prefix-command)
(keymap-set (n m v) override-global-map ";" #'execute-extended-command)

(defvar oo-leader-map (make-sparse-keymap))
(define-prefix-command 'oo-leader-prefix-command 'oo-leader-map)

(keymap-set oo-leader-map ";" #'+org-agenda-day-view)
(keymap-set oo-leader-map oo-normal-leader-key #'execute-extended-command)
(keymap-set oo-leader-map "b" #'oo-buffer-prefix-command :wk "buffer")
(keymap-set oo-leader-map "g" #'oo-git-prefix-command :wk "git")
(keymap-set oo-leader-map "l" #'oo-git-prefix-command :wk "git")
(keymap-set oo-leader-map "w" #'oo-window-prefix-command :wk "window")
(keymap-set oo-leader-map "a" #'oo-app-prefix-command :wk "app")
(keymap-set oo-leader-map "p" #'oo-package-prefix-command :wk "package")
(keymap-set oo-leader-map "f" #'oo-find-prefix-command :wk "find")
(keymap-set oo-leader-map "h" #'oo-help-prefix-command :wk "help")
(keymap-set oo-leader-map "e" #'oo-emms-prefix-command :wk "emms")
(keymap-set oo-leader-map "t" #'oo-toggle-prefix-command :wk "toggle")
(keymap-set oo-leader-map "q" #'oo-quit-prefix-command :wk "quit")
;;;;; window
(defvar oo-window-map (make-sparse-keymap))
(define-prefix-command 'oo-window-prefix-command 'oo-window-map)

(keymap-set oo-window-map "v" #'split-window-horizontally)
(keymap-set oo-window-map "h" #'split-window-vertically)
(keymap-set oo-window-map "b" #'balance-windows)
(keymap-set oo-window-map "M" #'maximize-window)
(keymap-set oo-window-map "d" #'delete-window)
(keymap-set oo-window-map "D" #'delete-other-windows)
(keymap-set oo-window-map "k" #'display-buffer)
(keymap-set oo-window-map "u" #'winner-undo)
(keymap-set oo-window-map "t" #'transpose-frame)
(keymap-set oo-window-map "s" #'ace-swap-window)
(keymap-set oo-window-map "w" #'ace-window)
(keymap-set oo-window-map "j" #'ace-window)
(keymap-set oo-window-map "o" #'ace-window)
(keymap-set oo-window-map "S" #'burly-bookmark-windows)
(keymap-set oo-window-map "b" #'burly-bookmark-windows)
;;;;; git
(defvar oo-git-map (make-sparse-keymap))
(define-prefix-command 'oo-git-prefix-command 'oo-git-map)

(keymap-set oo-git-map "b" #'vc-switch-branch)
(keymap-set oo-git-map "l" #'vc-switch-branch)
(keymap-set oo-git-map "s" #'magit-status)
(keymap-set oo-git-map "g" #'magit-status)
(keymap-set oo-git-map "p" #'magit-push)
(keymap-set oo-git-map "c" #'magit-commit)
(keymap-set oo-git-map "B" #'magit-branch)
(keymap-set oo-git-map "n" #'oo-dwim-vc-action)
;;;;; org
...
(keymap-set oo-org-map "a" #'org-archive-subtree)
(keymap-set oo-org-map "l" #'org-clock-in-last)
(keymap-set oo-org-map "i" #'org-clock-in)
(keymap-set oo-org-map "k" #'org-clock-in)
(keymap-set oo-org-map "o" #'org-clock-out)
(keymap-set oo-org-map "s" #'org-add-note)
(keymap-set oo-org-map "n" #'org-add-note)
(keymap-set oo-org-map "p" #'+org-capture-plain)
;;;;; app
(defvar oo-app-map (make-sparse-keymap))
(define-prefix-command 'oo-app-prefix-command 'oo-app-map)

(keymap-set oo-app-map "j" #'+org-capture-todo)
(keymap-set oo-app-map "n" #'notmuch)
(keymap-set oo-app-map "e" #'eshell)
;; (keymap-set oo-app-map "s" nil :wk "screenshot")
(keymap-set oo-app-map "s r" #'escr-region-screenshot)
(keymap-set oo-app-map "s f" #'escr-frame-screenshot)
(keymap-set oo-app-map "s w" #'escr-window-screenshot)
;;;;; toggle
...
(keymap-set oo-toggle-map "c" #'blink-cursor-mode)
(keymap-set oo-toggle-map "g" #'grugru)
(keymap-set oo-toggle-map "s" #'smartparens-mode)
(keymap-set oo-toggle-map "r" #'oo-load-random-theme)
(keymap-set oo-toggle-map "t" #'load-theme)
(keymap-set oo-toggle-map "h" #'whitespace-mode)
(keymap-set oo-toggle-map "W" #'whitespace-mode)
(keymap-set oo-toggle-map "w" #'widen)
(keymap-set oo-toggle-map "l" #'display-line-numbers-mode)
(keymap-set oo-toggle-map "u" #'toggle-truncate-lines)
(keymap-set oo-toggle-map "n" #'oo-dwim-narrow)
(keymap-set oo-toggle-map "i" #'iedit-mode)
(keymap-set oo-toggle-map "e" #'eval-expression)
(keymap-set oo-toggle-map "f" #'oo-set-font-face)
(keymap-set oo-toggle-map "r" #'read-only-mode)
(keymap-set oo-toggle-map "d" #'toggle-debug-on-error)
(keymap-set oo-toggle-map "P" #'profiler-stop)
;;;;; buffer
(defvar oo-buffer-map (make-sparse-keymap))
(define-prefix-command 'oo-buffer-prefix-command 'oo-buffer-map)

(keymap-set oo-buffer-map "x" #'kill-current-buffer)
(keymap-set oo-buffer-map "b" #'switch-to-buffer)
(keymap-set oo-buffer-map "j" #'next-buffer)
(keymap-set oo-buffer-map "k" #'previous-buffer)
(keymap-set oo-buffer-map "b" #'switch-to-buffer)
;;;;; help
(defvar oo-help-map (make-sparse-keymap))
(define-prefix-command 'oo-help-prefix-command 'oo-help-map)

(keymap-set oo-help-map "m" #'describe-mode)
(keymap-set oo-help-map "l" #'describe-function)
(keymap-set oo-help-map "f" #'describe-function)
(keymap-set oo-help-map "j" #'describe-variable)
(keymap-set oo-help-map "v" #'describe-variable)
(keymap-set oo-help-map "h" #'describe-variable)
(keymap-set oo-help-map "C" #'describe-char)
(keymap-set oo-help-map "k" #'describe-key)
;;;;; find
(defvar oo-find-map (make-sparse-keymap))
(define-prefix-command 'oo-find-prefix-command 'oo-find-map)

(keymap-set oo-find-map ";" #'save-buffer)
(keymap-set oo-find-map "o" #'find-file)
(keymap-set oo-find-map "E" #'oo-open-emacs-config)
(keymap-set oo-find-map "I" #'oo-open-emacs-init-file)
(keymap-set oo-find-map "L" #'oo-open-emacs-lisp-dir)
(keymap-set oo-find-map "G" #'rgrep)
(keymap-set oo-find-map "p" #'consult-yank-pop)
(keymap-set oo-find-map "k" #'consult-bookmark)
(keymap-set oo-find-map "b" #'consult-bookmark)
(keymap-set oo-find-map "l" #'consult-line)
(keymap-set oo-find-map "h" #'consult-outline)
(keymap-set oo-find-map "g" #'consult-grep)
(keymap-set oo-find-map "z" #'ace-link)
(keymap-set oo-find-map "b" #'burly-open-bookmark)
(keymap-set oo-find-map ";" #'save-buffer)
(keymap-set oo-find-map "i" #'imenu)
(keymap-set oo-find-map "j" #'oo-dwim-narrow)
(keymap-set oo-find-map "n" #'oo-new-buffer)
(keymap-set oo-find-map "o" #'find-file)
(keymap-set oo-find-map "f" #'switch-to-buffer)
(keymap-set oo-find-map "d" #'display-buffer)
(keymap-set oo-find-map "a" #'find-library)
(keymap-set oo-find-map "f" #'switch-to-buffer)
(keymap-set oo-find-map "d" #'pop-to-buffer)
;;;;; quit
...
(keymap-set oo-quit-map "E" #'restart-emacs-start-new-emacs)
(keymap-set oo-quit-map "r" #'restart-emacs)
...
(keymap-set oo-emms-map "p" #'emms-pause)
(keymap-set oo-emms-map "P" #'emms-stop)
(keymap-set oo-emms-map "r" #'emms-toggle-repeat-track)
(keymap-set oo-emms-map "R" #'emms-toggle-repeat-playlist)
(keymap-set oo-emms-map "v" #'emms-volume-lower)
(keymap-set oo-emms-map "V" #'emms-volume-raise)
(keymap-set oo-emms-map "s" #'emms-seek-to)
;;;;; package
