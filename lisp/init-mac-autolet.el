  ...
  (if (symbolp match-form)
    ...
    (cl-flet ((list-marker-p (it) (and (symbolp it) (equal ?& (aref (symbol-name it) 0)))))
      (let* ((binds (o-destruc-pcase-bindings match-form value))
             (non-gensyms (cl-remove-if #'list-marker-p (o-destruc-match-form-symbols match-form)))
             (all (o-destruc-match-form-symbols (mapcar #'car binds)))
      ...
      (cond ((equal (car frame) t)
            ...
            ((and (equal (cadr frame) 'o-set)
             ...
             (if (sequencep (caddr frame))
                 (dolist (symbol (reverse (o-destruc-match-form-symbols (caddr frame))))
