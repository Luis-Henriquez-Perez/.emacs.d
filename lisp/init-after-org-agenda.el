;;; init-after-org-agenda.el --- Configure `org-agenda' -*- lexical-binding: t; -*-
;;
...
Return -1 if B is more overdue than A.  Otherwise return 0."
  ...
  (cond ((or (and (not diff-a) (not diff-b))
             ...
             (and diff-a diff-b (cl-plusp diff-a) (cl-plusp diff-b))
             (and diff-a diff-b (= diff-a diff-b)))
         0)
        ((and diff-a (cl-minusp diff-a) (not diff-b))
         1)
        ((and (not diff-a) diff-b (cl-minusp diff-b))
         -1)
        ((and (cl-minusp diff-a) (cl-plusp diff-b))
         1)
        ((and (cl-plusp diff-a) (cl-minusp diff-b))
         -1)
...
with no deadline."
  ...
  (cond ((and diff-a (cl-plusp diff-a) (not diff-b))
        ...
        ((and diff-a (cl-plusp diff-a) diff-b (cl-minusp diff-b))
         1)
        ((and diff-b (cl-plusp diff-b) diff-a (cl-minusp diff-a))
         -1)
        (t
         0)))
...
(defun +org-agenda-day-view ()
  ...
  (let ((org-agenda-custom-commands
    ...
    (org-agenda nil "_")))
;;; provide
(provide 'init-after-org-agenda)
;;; init-after-org-agenda.el ends here
