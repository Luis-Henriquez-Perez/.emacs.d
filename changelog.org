...
be normalized. It is difficult to read the data structure at a glance because
each cond clause is a long list of forms. It is harder to determine what is done
with a particular keyword because many of the clause predicates are of the form
~(memq leaf--key '(KEYS))~. If I wanted to access the form to be evaluated by
said key, for example, how do I do that?. If you wanted to add you're own
normalizer, it takes more code. This should really be an alist mapping leaf keys
to normalizer functions.

The benefit of using functions as opposed to forms. More difficult to debug.
Elisp has many useful methods for debugging functions. The flexibility of
functions allso allows you to.

*** why not =use-package=
:PROPERTIES:
:ID:       773bf0d9-851d-4052-b6a9-38102f3adee5
:END:

The simple answer is it's too annoying to define new keywords. In =def-package=, I
try to follow the least knowledge principle wherein the user should have to know
the bare minimum information possible to add a new custom three words.

#+begin_src emacs-lisp
(def-package-key! mode ((regexp . mode))
  `(add-to-list 'auto-mode-alist (cons ,regexp ',mode)))#+end_src

Yes, you can look up the documentation and play with it until you finally figure
it out, but the point is you shouldn't have to do that. You shouldn't have to
have knowledge of =use-package-concat= and =use-package-process-keywords= just. The
situation arose where I know exactly what form I want to add. Something as
simple as adding a lisp form to a list should not be this complicated. This has
the consequence of making it difficult for the user to read.
