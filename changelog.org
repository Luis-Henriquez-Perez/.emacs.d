...
=use-package= is likely to be added to emacs. Almost a decade old, It is the more
mature of the two packages and consequently has more community support (blog
post, auxiliary packages, etc.) =use-package= rightly uses higher-order functions
(the normalizer and handler) to control the expansion of new keywords as opposed
to raw forms like leaf. Therefore, I think it is best to switch from leaf to
use-package.

** =void-add-hook= is used to add new hook keywords
:PROPERTIES:
:ID:       03caab25-1ab2-4aaa-8040-03b48d5b14be
:END:
