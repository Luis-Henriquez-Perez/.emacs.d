...
It is useful to apply some keywords to all emacs packages such as ensure. I want
to "ensure" all my packages. This means to.
...
(defun def-package-set-alias (alias original)
  ...
  (map-put! def-package-alias-alist alias original))
#+end_src
