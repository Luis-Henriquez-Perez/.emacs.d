...
[[helpfn:doct][doct]] only returns what should be added into =org-capture-templates=, but it
does not actually add it. As it is convenient to declare it and add it in all in
one go. Additionally, it removes any capture templates with the same key. This
is so that I can freely re-evaluate it without cluttering my capture templates
...
same key as the one being added."
    (let* ((capture-item (doct declarations))
           (keys (-map #'-first-item capture-item)))
      (message "KEYS: %s | OTHER: %s" keys (-map #'car org-capture-templates))
      (setq org-capture-templates
            (append capture-item
                    (--remove (-any-p (-partial #'-contains-p it) keys)
...
Create"
    (let* ((url (exwm::get-url-from-firefox))
           ...
           (pdf-name (s-replace "/" "~" (nth 1 (s-match title-regexp exwm-title))))
           (pdf-path (format "%s%s.pdf" VOID-SCREENSHOT-DIR pdf-name))
  ...
  (org-capture:add-to-templates
   (list "Website"
         :keys "w"
         :contexts (list :in-buffer "[^z-a]+ - \\(?:firefox\\|ice\\(?:cat\\|weasel\\)\\)\\'")
