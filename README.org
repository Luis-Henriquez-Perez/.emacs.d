...
#+begin_src emacs-lisp
(defun void:new-hook-name (target hook)
...
#+begin_src emacs-lisp
(defun void:add-hook (target hook &optional depth local expire-fn)
  "Add a VOID hook to add TARGET."
  (let ((new-name (void:new-advice-name advisor advisee)))
    (defalias new-name advisor)
    (advice-add advisor :after new-name)
    (when expire-fn
      (void:add-expiring-advice hook expire-fn))))
#+end_src
...
****** TODO unbind target of expiring advice
:PROPERTIES:
:ID:       8d8e1ef4-bb49-41ed-b798-55c6295707cd
:END:

#+begin_src emacs-lisp
(defvar void:unbind-advice-target-p t "Whether to unbind target of expiring advice.")
#+end_src
...
(defun void:add-advice (target where advice &optional props expire-fn)
  ...
  (let ((advice-name (void:new-advice-name target advice)))
    (defalias advice-name target)
    (advice-add target :after advice-name)
    (when expire-fn
      (void:add-expiring-advice advice-name expire-fn :unbind))))
#+end_src
