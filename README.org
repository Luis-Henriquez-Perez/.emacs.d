...
[[][use-package]]. Here I try to simplify its system of adding leaf keywords by
adding more expected
...
#+begin_src emacs-lisp
(defvar leaf~process-vars '(raw name key keyname value body)
...
#+begin_src emacs-lisp
(defun leaf~vars ()
  (-map (-partial #'void-symbol-intern 'leaf--) leaf~process-vars))
#+end_src
...
(defun leaf~anaphoric-vars ()
  (-map #'void-anaphoric-symbol leaf~preprocess-vars))
#+end_src
...
(defmacro defleaf! (name args &rest body)
  ...
  (declare (indent defun))
  (-let* (((where place) args)
          ((name asterix) (cdr (s-match "\\([^z-a]+\\)\\(\\*\\)")))
          (keyword (void-keyword-intern name))
          (fname (intern (format "leaf~~apply-%s" name))))
    `(progn
       ,(alet (,fname ,@(leaf~vars))
          `(leaf~insert-keyword ,keyword ',where ,place ',it))

       (defun ,fname ,(leaf~anaphoric-vars)
         (-let ((,(leaf~vars) ,(leaf~anaphoric-vars)))
           (ignore ,@(leaf~vars) ,@(leaf~anaphoric-vars))
           (list (progn ,@body) (macroexp-progn <body>)))))))
#+end_src
...
**** use =after!= in leaf-defer                                         :disabled:
:PROPERTIES:
...
(defleaf! defer (before leaf-defer :leaf-defer)
  (if (and <body> (not (null (car <value>)))
...
#+begin_src emacs-lisp
(defleaf! setq (before setq (var . value))
  `(setq ,var ,value))
#+end_src
...
#+begin_src emacs-lisp
(defleaf! before-call (before :hook)
  (let ((advice (void-symbol-intern 'load- )))
    `(progn (void-advice-add ,<value> :before ,advice (-const t))
...
#+begin_src emacs-lisp
(defleaf! straight (before ensure)
...
(defleaf! popup (before :mode)
  `(push ',<value> display-buffer-alist))
#+end_src
...
#+begin_src emacs-lisp
(defleaf! alias (before :init (alias . def))
