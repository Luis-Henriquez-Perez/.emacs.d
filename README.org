...
This function has the side-effect of inserting KEY before ANCHOR (or after ANCHOR if AFTER is non-nil.)"
...
(defun use-package-normalize/:before-call (name keyword args)
  (aprog1 (use-package:as-one keyword args #'symbolp "a symbol")
    (message "UP after: %S" it)))

(defun use-package-handler/:before-call (name _keyword features rest state)
  (message "HAND: %S" features)
  (use-package-concat
   (use-package-process-keywords name rest state)
...
(defun use-package-normalize/:alias (name keyword args)
  (let ((message "cons cell"))
    (use-package:as-one keyword args #'-cons-pair-p message)))
