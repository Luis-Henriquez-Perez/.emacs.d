...
(defun use-package-normalize/:setq (name keyword args)
  (when (and (= (length args) 1) (not (-cons-pair-p (car args))))
    (setq args (car args)))
  (unless (-all-p #'-cons-pair-p args)
    (thread-last (-find (-not #'-cons-pair-p) args)
      (format "All arguments should be cons cells. %S isn't though." args)
      (use-package-error)))
  args)
...
(defadvice! abbrev-hooks-and-modes (:override use-package-normalize/:hook)
  (thread-last (use-package:args-as-one <args>)
    (--map-when (not (-cons-pair-p it)) (cons it nil))
    (map-keys-apply #'use-package:ensure-hook)
    (map-values-apply (-rpartial #'use-package:ensure-mode <name>))))

(defun use-package:ensure-mode (val package)
  (or val
      (alet (not (string-match-p "-mode\\'" package))
        (void-symbol-intern <name> (when it "-mode")))))
...
#+begin_src emacs-lisp
(defadvice! use-void-add-hook (:override use-packag-handler/:hook)
  (use-package-concat
   (use-package-process-keywords <name> <rest> <state>)
   (-list <args>)
...
(defun use-package-handler/:popup ())
#+end_src
...
#+begin_src emacs-lisp
(defun use-package:args-as-one (args)
  (if (and (= 1 (length args))
           (and (listp (car args))
                (not (-cons-pair-p (car args)))))
      (car args)
    args))
#+end_src
