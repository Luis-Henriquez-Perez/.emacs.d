...
The same as `defhook', but allow multiple hooks and for keyword arguments to be
added before the docstring."
...
This addition to be font-lock causes =defhook= to be colored like =defun=.
  ...
  (defhook! auto-revert-auto-revert-visible-buffers (focus-in-hook after-save-hook)
    ...
    (--each (-uniq (mapcar #'window-buffer (window-list)))
      (with-current-buffer it
        (void:after-switch-buffer-hook&auto-revert-buffer-maybe)))))
#+end_src
