...
(defun use-package-handler/:trigger (name _keyword features rest state)
  (use-package-concat
   (use-package-process-keywords name rest state)
   ;; I need to get the functions autoloaded by entry point keywords.
   (message "rest: %S" rest)
   (message "commands: %S" (plist-get rest :commands))
   nil
...
(define-capture-template! Website ()
  :immediate-finish t
  :after-finalize #'exwm:save-browser-webpage
  (let* ((url (exwm:get-browser-url))
         (pdf-name (s-match ))
