...
containing the arguments. For example, =:foo 1 2 3 4= versus =:foo (1 2 3 4)=. This
flexibility has its drawbacks because in lisp.
...
This function strives to be more concise than [[][]] by automatically. Also,
unlike the latter, it always returns a list.
...
that I feel are not covered by default =use-package= keywords. Note that, as it
says in the [[][]], defining a keyword takes three parts.
...
(defadvice! print-output-on-debug (:around straight-use-package)
  (let ((package (or (car-safe (car <args>))
                     (car <args>))))
...
** pinentry
:PROPERTIES:
:ID:       b52e958b-02a5-4ca3-84f4-b39c824f4fc1
:END:
