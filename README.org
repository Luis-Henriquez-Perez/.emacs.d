...
=defhook= provides an alternative, declarative mechanism for defining hooks.
...
#+begin_src emacs-lisp
;; assert-rtn is a dependency of defhook.
(use-package! assert-rtn :require t)
(use-package! defhook
  :require t
  :setq
  (defhook-user-prefix . "")
...
***** wrapper around defhook                                          :disabled:
:PROPERTIES:
...
The same as `defhook', but allow multiple hooks in arguments.
Also allow for arguments to be added bellow the docstring."
  ;; if there is more than one hook, separate it into a different `defhook' form.
  (let ((hook (car args))
        (hooks (-take-while (-not #'keywordp) (cdr args)))
        (specs (-drop-while (-not #'keyword-p) args))
        (docstring (if (stringp (car body)) (pop body) nil))
        (other-args body))
    `(progn
       (defhook ,name ,(cons hook specs)
         ,docstring
         ,@body)
       (defhook! ,name ,(append hooks specs)
         ,docstring
         ,@body))))
#+end_src

***** customize informational messages                                :disabled:
:PROPERTIES:
:ID:       26783153-3517-4079-89d8-4bd862c52909
:END:

The messages `defhook' provides are informational but long and in complete
sentence. I would rather have something quick and concise.

#+begin_src emacs-lisp
(defadvice! better-message (fn hook op began-in-hook hook-ending &rest hook-args)
  ""
...
The default message provided by =defhook= is long and wordy especially
considering that.

#+begin_src emacs-lisp
(defadvice! provide-concise-hook-message (fn hook &rest _)
  "Use a more concise message for `defhook'."
  :override defhook-executed
  (void:log "%s exected on %s" fn hook))
#+end_src
...
#+begin_src emacs-lisp
(defhook! wrap-text-in-message (mu4eview-mode-hook)
