...
Following [[][past examples]], I initially opted for allowing keyword arguments
in the "function args" part of defun-like macros. This is fine when there's only
one keyword argument, but any more and it starts to get crowded. It doesn't help
that emacs functions tend towards longer names due to a lack of namespaces.
Therefore, I support keyword args in the function body.

#+begin_src emacs-lisp
(defun void--keyword-macro-args (body)
  "Return list of (KEYWORD-ARGS BODY)."
  (list (--unfold (when (keywordp (car it))
                    (cons (-take 2 it)
                          (setq body (cddr it))))
                  body)
        body))
...
so often used in macros.

***** a word that contains an anaphoric keyword
:PROPERTIES:
:ID:       4df8da60-621b-4fe7-b451-5cd7b44489e0
:END:

#+begin_src emacs-lisp
(defun void--contains-anaphoric-p (symbol)
  "Return non-nil if symbol contains an anaphora."
  (s-matches-p (rx "<" (1+ (not white)) ">")
               (symbol-name symbol)))
#+end_src
...
#+begin_src emacs-lisp
(defun void--format-equiv (symbol)
  "Return what will replace the symbol."
  (s-match (rx (group "<" (1+ (not (white))) ">")) symbol)
  (s-replace-regexp regexp "%s" symbol)
  `(intern (format "something-%s-something") symbol))
#+end_src

***** format keyword
:PROPERTIES:
:ID:       1c8275ad-166e-4cc0-9f1c-c42de2adafdb
:END:

#+begin_src emacs-lisp
(defmacro format! (name args &rest body)
  "Uses anaphora to format."
  (-tree-map ))
#+end_src
...
#+begin_src emacs-lisp
(use-package! ytdl
  :ensure-system-package youtube-dl
  :setq
...
***** capture file
:PROPERTIES:
:ID:       459ccec8-37dc-4e7e-9382-9737ff0c3bf6
:END:

I like to keep things simple. If I'm already going to be filtering captures,
then why segregate them into multiple files? That just makes it harder because I
have to consider which ones are in which file before filtering.

#+begin_src emacs-lisp
(defconst VOID-CAPTURE-FILE (concat VOID-ORG-DIR "capture.org")
  "File where all capture templates are filed to.")
#+end_src
