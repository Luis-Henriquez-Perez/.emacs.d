...
#+begin_src emacs-lisp
(dolist (package '(leaf-keywords leaf dash-functional anaphora))
...
Bare symbols in BINDINGS are equivalent to:
  ...
  (let* ((bind-fn (lambda (bind)
                    (if (consp bind)
                      ...
                      (cons bind bind))))
         (names-and-forms (mapcar bind-fn bindings))
         (names (mapcar #'car names-and-forms))
         (forms (mapcar #'cdr names-and-forms))
         (symbols (mapcar (lambda (it) (make-symbol (symbol-name it))) names)))
...
*** let macro
:PROPERTIES:
:ID:       29860be2-6f3f-4f3d-aea7-21d3793183fc
:END:

#+begin_src emacs-lisp
(defmacro let! (bindings &rest body)
  (cl-labels ((add-commas (form) (if (atom it) (list '\, it) (add-commas it))))
    `(pcase-let ,bindings ,@body)))

(cl-labels ((add-commas (it) (if (atom it) (list '\, it) (add-commas it))))
  (add-commas '(a b c (d e f))))

(--tree-map (if (atom it) (list '\, it) it) '(a b c (d e f)))

(mapcar (lambda (it) (if (symbolp it) (list '\, it) )) '(a b c))
...
HOOK-FN is a function named with Void naming conventions."
  (thread-last (symbol-name hook-fn)
...
(defun void-hook-name (var hook)
  ...
  (funcall (apply-partially #'void-symbol-intern var '&)
           (or (thread-last (symbol-name hook)
...
(defun void-add-hook (var hook &optional depth local expire-fn)
  ...
  (let* ((new-hook (void-hook-name var hook))
    ...
    (when expire-fn
      (thread-last (void-expire-advice hook expire-fn t)
...
\"advisee\"."
  (thread-last (symbol-name advice)
...
Advice name is of the form FN@ADVICE."
  (funcall (apply-partially #'void-symbol-intern fn '@)
           (or (thread-last (symbol-name advice)
...
This function generates a new."
  (let* ((new-advice (void-advice-name target advice))
    ...
    (when expire-fn
      (thread-last (void-expire-advice target expire-fn)
...
\(fn ACTION (WHERE TARGET &rest TARGETS) &rest BODY)"
  (-let* (((where target . other-args) args)
    ...
    `(define-advice! ,name (,where ,advice-args ,target ,@other-args)
       (ignore <args>)
       (cl-progv (thread-last (help-function-arglist #',target t)
                   (cl-remove-if
                    (lambda (it) (s-starts-with-p "@" (symbol-name it))))
                   (mapcar
                    (lambda (it) (intern (format "<%s>" (symbol-name it))))))
           <args>
         ,@body))))
#+end_src
...
\(fn ACTION (WHERE &optional ARGS-LIST TARGET &rest TARGETS) &rest BODY)"
  (-let* ((symbols-only (lambda (it) (and (symbolp it) (not (keywordp it)))))
    ...
    `(progn
       ,@(mapcar (lambda (fn)
                   `(,advice-macro ,name (,@before ,fn ,@after) ,@body))
                 fns))))
#+end_src
...
(defmacro after! (features &rest body)
  ...
  (pcase (car-safe features)
    ('or
     (macroexp-progn
      (mapcar (lambda (it) `(after! ,it ,@body)) (cdr features))))
    ('and
     (void-wrap-form (mapcar (lambda (it) `(after! ,it)) (cdr features))
                     (macroexp-progn body)))
...
(defhook! start-in-char-mode (exwm-manage-finish-hook)
  ...
  (when (cl-some (lambda (it) (string-prefix-p it exwm-instance-name))
                 exwm:char-mode-apps)
...
(defun defsnippet:new-snippet (&rest plist)
  ...
  (let! (((&plist :name :mode :body :placeholders) plist)
    ...
    (cl-assert (cl-notany #'null (list name mode body)))
...
(defun defsnippet:compute-body (body placeholders)
  ...
  (alet (mapcar (-lambda ((number name form))
                (if (eq '_  name)
...
(use-package! ivy-avy
  ...
  :commands ivy-avy
  :init
  (let ((path (cl-find (apply-partially #'s-matches-p (rx "/ivy" eos)) load-path)))
    (concat path "-avy")
...
(defun pass:email-account-plist (email)
  ...
  (shut-up!
    (thread-last (cdr (password-store-parse-entry email))
         (mapcar #'car)
...
ELEMENTS is a list of headline elements (see `org:element-headline-elements')."
  (awhen (seq-find (-some-p (-partial #'org:element-contains-point-p point) it)
                   (thread-last (cdr elements)
                     (-group-by #'org:element-paragraph-p)
                     (seq-filter (-compose #'org:element-paragraph-p #'car))))
...
(let ((font-dir
  ...
  (unless (cl-every (lambda (it) (file-exists-p (concat font-dir it)))
                    '("all-the-icons.ttf"
                      "file-icons.ttf"
                      "fontawesome.ttf"
