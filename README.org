...
(defmacro defleaf! (name args &rest body)
  ...
  (-let* (((where place alias defer normalizer) args)
          ...
          (fname (void-symbol-intern 'leaf~apply- keyword))
          (leaf-vars (-map (-partial #'void-symbol-intern 'leaf--) leaf-vars))
          (anaphoric-vars (-map #'void-anaphoric-symbol leaf-vars)))
    `(progn
       (leaf~insert-keyword ,keyword)
