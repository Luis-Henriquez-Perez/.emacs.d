...
:END:
  ...
  the name of a built-in or external feature, greatly simplifies the task of
  programmatically loading my files and provides a level of transparency to any
  reader, ensuring that they know the precise order my files at a glance.
  Additionally, I divide these files into "init" files that are evaluated
  immediately, and "config" files which are evaluated after some feature is
  loaded.  This division lends itself to the the deferred loading of features
  while also allowing me to write declarative code in each file as opposed to
  code encased with [[/home/luis/.config/documentation/29.4-helpful--eval-after-load.html][eval-after-load]].
