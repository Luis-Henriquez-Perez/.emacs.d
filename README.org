...
- Extensive abbrev configuration with well over 1200 abbrevs
...
- Suppress errors raised in hooks with custom hook functions
- Extensively log events using my own custom logging function
- Lightweight, dependency-free, profiling of each file loaded
- Gracefully error handling when loading initialization files
- Plain and transparent file loading order
- Beautiful, hand-crafted modeline with [[https://github.com/milkypostman/powerline][powerline]] separators
...
- Simple and explicit loading order and unique file names via numeric prefixes
* File Structure
:PROPERTIES:
...
:END:
  ...
  reflect the order in which they are loaded.  A numeric prefix not only
  virtually guarantees that file names will not conflict with the name of a
  built-in or external feature, but also greatly simplifies the task of
