...
Each key can take values of a different form. For example, the [[][]] can take a
cons cell of the form ~(hook . value)~, or a just the hook variable, or a list of
these. Functions created for customizing a keyword are easier to write and
easier to read if they just assume one standard structure for their value. To
accommidate different ways users can pass in values to. The reason that the
