...
#+begin_src emacs-lisp
(defun void:new-advice-name (advisor advisee)
...
#+begin_src emacs-lisp
(defun void:get-advisee (advisor)
  "Return the advisee that ADVISOR advices."
  (s-match (rx)
           (symbol-name advisor))
  )
#+end_src
...
#+begin_src emacs-lisp
(defun void:add-advice (advisor where advisee &optional props expire-fn)
  "Add a VOID advice to add ADVISEE."
  (let ((new-name (void:new-advice-name advisor advisee)))
    (defalias new-name advisor)
    (advice-add advisor :after new-name)
    (when expire-fn
      (void:add-expiring-advice new-name expire-fn :unbind))))
#+end_src
...
#+begin_src emacs-lisp
(defun void:add-expiring-advice (advisee expire-fn &optional unbind)
  "Add an advice which causes ADVISEE."
  (let ((expire-advice (void:new-advice-name advisee 'expire)))
    (fset expire-advice
          (lambda (orig-fn &rest args)
            (aprog1 (apply orig-fn args)
              (when (funcall expire-fn it)
                (advice-remove )
                (advice-remove advisee expire-advice)
                (fmakunbound expire-advice)
                (when unbind (fmakunbound advisee))))))))
#+end_src
