...
Add a hook that follow naming conventions.

#+begin_src emacs-lisp
(defun void:add-hook (var hook &optional depth local expire-fn)
  "Add a HOOKS to VARS."
  (let ((hook (void:hook-name var hook)))
    (add-hook var hook depth local)
    (when expire-fn
      (->> (void:expire-advice hook expire-fn t)
           (advice-add hook :around)))))
#+end_src
...
=defhook!= provide a declarative way of setting hooks.
...
(defmacro defhook! (name args &rest body)
  ...
  (let* ((hooks (-take-while (-not #'keywordp) args))
         (hook-args (-find #'listp args))
         (depth (or (plist-get :depth args) (plist-get :append args)))
         (local (plist-get :local args))
         (docstring (if (stringp (car body)) (pop body) nil)))
    `(progn
       ,@(-map (-lambda ((fn . hook))
                 `(progn (add-hook ',hook #',fn ,depth ,local)
                         (defun ,fn ,hook-args ,docstring
...
(defhook! add-defhook-keywords (emacs-lisp-mode-hook)
  ...
  (alet `((,(rx (group "defhook!") (1+ white) (group (1+ (not white))))
...
**** defhookvar
:PROPERTIES:
:ID:       55f1a3fa-ca79-4a3d-8765-fa709897ff0b
:END:

This macro provides a way of defining a hook variable.

#+begin_src emacs-lisp
(defmacro defhookvar! (name triggers docstring)
  "Define a hook variable."
  `(progn
     (defvar ,name nil ,docstring)
     (--each ,triggers
       (if (functionp it)
           (void:add-advice it :around run-hook)
         (void:add-hook it)))))
#+end_src
...
**** generic advices
:PROPERTIES:
:ID:       b41b0bfa-83a7-4aa4-97ca-80e72b52cb08
:END:

This headline is for generic advices that I will use all the time and for many
different functions. These functions will not be used directly. Instead, they
will be used via [[helpfn:void:advice-add][void:advice-add]].

***** boost gc
:PROPERTIES:
:ID:       fd8d6c00-5f07-4c22-8268-7497d36534a6
:END:

Boosting garbage collection for.

#+begin_src emacs-lisp
(defun void::boost-gc-threshold-advice (orig-fn &rest args)
  "Boost garbage collection for the duration of ORIG-FN called with ARGS.
Set the values of `gc-cons-threshold' to `VOID:GC-CONS-THRESHOLD-MAX' and
`gc-cons-percentage' to `VOID:GC-CONS-PERCENTAGE-MAX' for the duration of
ORIG-FN."
...
***** silence output
:PROPERTIES:
:ID:       4e6463cc-c203-4193-bfff-eadc0331ed89
:END:

Many emacs packages (both internal and external) print messages to output.

#+begin_src emacs-lisp
(defun void::silence-output-advice (orig-fn &rest args)
  "Silence output for the duration of ORIG-FN called with ARGS."
  (quiet! (apply orig-fn args)))
#+end_src

***** use =void:log= instead of =message=
:PROPERTIES:
:ID:       271549f3-dcba-481a-87b4-ef01089664fc
:END:

Some packages such as [[id:c550f82a-9608-47e6-972b-eca460015e3c][idle-require]] message output that I'd like only when [[helpfn:void:debug-p][void:debug-p]]
is non-nil.

#+begin_src emacs-lisp
(defun void::use-void-log-advice (orig-fn &rest args)
  "Use `void:log' instead of `message' for ORIG-FN called with ARGS."
...
The only difference between this and [[][]] is that =NAME= and =SYM= are switched. I
feel that having.
...
=anaphoric-define-advice!= lets you omit the =lambda-args=. If you don't use arguments.
...
This takes care of allowing multiple advices and deciding between whether to use
=defadvice!= or =anaphoric-defadvice!=.
...
(defhook! add-defadvice-keywords (emacs-lisp-mode-hook)
  ...
  (alet `((,(rx (group "defadvice!") (1+ white) (group (1+ (not white))))
           (1 font-lock-keyword-face)
           (2 font-lock-function-name-face)))
    (font-lock-add-keywords nil it)))
#+end_src
...
(defhook! add-anaphoric-symbols-font-lock (emacs-lisp-mode-hook)
  ...
  (alet `(,(rx symbol-start (group "<" (1+ (not white)) ">") symbol-end)
