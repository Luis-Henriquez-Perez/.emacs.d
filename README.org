...
<p align="center">
  <a href="#principles-and-goals">Principles and Goals</a> •
...
Theme: [[https://github.com/protesilaos/ef-themes][ef-light]] Font: [[https://www.recursive.design/][recursive mono]]
...
- *performant* (WIP)
  ...
  snappy.
- *introspectable* (not a real word)
...
- Extensive abbrev configuration with over [[file:lisp/999-plain-text-abbrevs.el][6500 abbrevs]]
- Auto-committing an saving abbrevs
...
- Mode-line and evil cursor faces [[file:.local/share/screenshots/2025-04-27-160359__emacs_dynamic_faces.png][dynamically adapt]] to theme
...
- A rich set of custom hooks for lazy loading =o-first-input-hook=, =o-first-file-hook=
* File Structure
:PROPERTIES:
:ID:       20240408T164104.628646
:END:

  I designed the file structure of my configuration to be simple, transparent,
  and to lend itself to deferred loading.  I prefix all file names (except the
  =init.el=) with a three digit number that corresponds to the order in which they
  are loaded.  This virtually guarantees that file names will not conflict with
  the name of built-in or external feature, greatly simplifies the task of
  programmatically loading my files and provides a level of transparency by
  ensuring that at a glance any reader knows the precise load order.  Moreover,
  I divide these files into "init" files that are evaluated immediately, and
  "config" files which are evaluated after some feature is loaded.  This
  division lends itself to the the deferred loading of features while also
  allowing me to write declarative, straightforward code (as opposed to the
  popular alternative: encasing code with [[/home/luis/.config/documentation/29.4-helpful--eval-after-load.html][eval-after-load]]).


  Below is a quick synopsis of each kind of file and a description.  I list the
  numerical prefix of the file relative to the Emacs configuration directory and
  a brief description of what the files with that particular prefix do.  Any =...=
  means "through" in the sense of specifying an interval.  So =000...002= means
  any of =000=, =001=, and =002=.

- *init.el* ([[file:./init.el][see it here]]) - /Initialize my Emacs configuration/.

  Add the lisp directory to the load-path, directly or indirectly load all
  needed files and set up necessary initial variables.  This is the "big bang"
  that sets everything else into motion.

- *lisp/000...002* - /Define needed variables and fundamental functions and macros./


- *lisp/020...050* - /Provide configuration functions and macros./


- *lisp/110* - /Install external packages with package.el./


- *lisp/130* - /Initialize FEATURE as specified by lisp/130-init-FEATURE.el./

  This means adding hooks or advices, setting variables, binding keys and autoloading
  functions.  Crucially, refrain from loading FEATURE or any other features
  unless absolutely necessary.

- *lisp/990* - /Configure FEATURE as specified by lisp/990-config-FEATURE.el./
  ...
  files starting with the number =9= are not loaded immediately via.

- *lisp/160* - /Set all of my keybindings./
...
- *lisp/127* - /Run custom hooks./

  Run the hooks that do not correspond to a particular package.

- *lisp/128* - /Some file I use but do not know what to do with yet./
...
- *lisp/999* - /Store my [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Abbrevs.html][abbrevs]]./
...
#+end_src
* Screenshots
[[.local/share/screenshots/2025-04-27-130548__emacs_moe-light.png]]
theme: [[https://github.com/kuanyui/moe-theme.el][moe-light theme]]
font: [[https://www.recursive.design/][recursive mono]]
[[.local/share/screenshots/2025-04-27-130749__emacs_moe-dark.png]]
theme: [[https://github.com/kuanyui/moe-theme.el][moe-dark theme]]
font: [[https://www.recursive.design/][recursive mono]]
* Acknowledgements
:PROPERTIES:
...
- [[https://config.phundrak.com/][Phundrak]]
