...
*** list symbols
:PROPERTIES:
:ID:       03c83098-da34-4a66-95d7-458555bc2a74
:END:

#+begin_src emacs-lisp
(defun void:list-symbols ()
  "List symbols."
  (interactive)
  )
#+end_src
...
**** TODO void function regexp
:PROPERTIES:
:ID:       8b83a560-30ad-4ef3-89a6-4e50eecaab65
:END:

#+begin_src emacs-lisp
(defconst void:function-name-regexp nil
  "Regexp for a function name.")
#+end_src

**** Hooks and Advices
:PROPERTIES:
:ID:       89c5152e-92f2-4282-a7c7-5a98f1edd4f6
:END:
...
****** TODO hook fn regexp
:PROPERTIES:
:ID:       0b1b992c-692d-406b-bf31-ead28bca5a19
:END:

#+begin_src emacs-lisp
(defconst VOID:HOOK-FN-REGEXP (rx (group (1+ anything)) "%" (group (1+ anything)))
  "Regular expression that defines how hooks are named.")
#+end_src
...
This returns the alias that is created.

#+begin_src emacs-lisp
(defun void:hook-name (target hook)
  "Return hook name that follows Void conventions."
  (alet (->> (symbol-name hook)
             (s-match (rx (1+ anything) (1+ ":") (group (1+ anything)) "-hook"))
             (nth 1))
    (intern (format "%s%%%s" (symbol-name target) it))))
#+end_src
...
#+begin_src emacs-lisp
(defun void:hook-target (hook)
  "Return the target of HOOK."
  (->> (symbol-name hook)
...
#+begin_src emacs-lisp
(defun void:add-hook (targets hooks &optional depth local expire-fn)
  "Add a HOOKS to TARGETS.
TARGETS, HOOKS, DEPTH, and LOCAL correspond to the arguments in
`add-hook'. Unlike `add-hook', TARGETS and HOOKS can be single
items or lists. EXPIRE-FN should be function. If passed in, EXPIRE-FN will"
  (dolist (target (-list targets))
    (dolist (hook (-list hooks))
      (add-hook target hook depth local)
      (when expire-fn
        (void:add-expiring-advice hook expire-fn t)))))
#+end_src

****** TODO add-hook interactively
:PROPERTIES:
:ID:       da6b44bc-9857-44ad-8063-ee48c22c55e0
:END:

#+begin_src emacs-lisp
(defun void/add-hook (hook advice)
  "Interactively add advice to HOOK."
  (interactive)
  (completing-read-multiple "Add a hook(s) to:" targets)
  (void:add-hook target (void:hook-name target hook)))
#+end_src

****** TODO remove hooks interactively
:PROPERTIES:
:ID:       0e840929-5f1b-46e7-993d-e6d7ba424f15
:END:

#+begin_src emacs-lisp
(defun void/remove-hook (hook advice)
  "Remove hooks"
  (interactive)
  )
#+end_src
...
(defmacro defhook! (name args &rest body)
  ...
  (-let* ((docstring (when (stringp (car body)) (pop body)))
          ((key targets . body) body)
          (append (eq key :append))
          (local (eq key :local)))
    `(prog1 (defun ,name ,args ,docstring ,@body)
       (void:add-hook targets hooks depth local))))
#+end_src

***** TODO advice
:PROPERTIES:
:ID:       01c00dd3-bbd5-45b7-8f3e-c8aba9091b18
:END:

The naming convention for advices is.

****** TODO advice-fn regexp
:PROPERTIES:
:ID:       4946ac50-69ea-49bb-b53e-25834962454d
:END:

#+begin_src emacs-lisp
(defconst VOID:ADVICE-FN-REGEXP (rx (group (1+ anything)) "@" (group (1+ anything)))
  "Regular expression that controls how advices are named.")
#+end_src
...
#+begin_src emacs-lisp
(defun void:new-advice-name (target advice)
  "Return new advice name based on ADVISOR and ADVISEE."
  (->> (s-match VOID:ADVICE-FN-REGEXP advisor)
       (nth 1)
       (void:symbol-intern advisee "@")))
#+end_src
...
#+begin_src emacs-lisp
(defun void:advice-p (fun)
  "Return t if FUN is a void advice."
  (->> (symbol-name fun)
       (s-matches-p VOID:ADVICE-FN-REGEXP)))
#+end_src

****** TODO add advice interactively
:PROPERTIES:
:ID:       5a4c11b9-a0f9-4239-8d05-3157bd18dba8
:END:

#+begin_src emacs-lisp
(defun void/add-advice ()
  "Interactively add an advice."
  (let ((functions))
...
****** TODO remove advice
:PROPERTIES:
:ID:       dcdcadb8-ac54-47f1-9349-acbc2496d8ed
:END:
...
(defun void:advice-target (advice)
  "Return the TARGET for advice."
  (->> (symbol-name advice)
       (s-match VOID:ADVICE-FN-REGEXP)
       (nth 1)
       (intern)))
#+end_src
...
****** TODO get advices
:PROPERTIES:
:ID:       15587752-6650-4312-94e2-86e484081ec7
:END:

#+begin_src emacs-lisp
(defun void:get-advices (target)
  "Return list functions advising TARGET."
  (advice-mapc))
#+end_src
...
#+begin_src emacs-lisp
(defun void:add-advice (targets where advices &optional props expire-fn)
  "Advise TARGETS with ADVICES."
  (dolist (target (-list targets))
    (dolist (advice (-list advices))
      (setq advice (void:advice-name target advice))
      (advice-add target where advice)
      (when expire-fn
        (void:add-expiring-advice advice expire-fn t)))))
#+end_src

****** TODO add expiring advice
:PROPERTIES:
...
#+begin_src emacs-lisp
(defun void:add-expiring-advice (target &optional expire-fn unbind)
  "Cause TARGET to expire when EXPIRE-FN returns true.
TARGET is a function. EXPIRE-FN is a function that returns true when TARGET
should expire."
  (let ((expire-advice (void:new-advice-name target 'expire))
...
(defmacro defadvice! (name args &rest body)
  ...
  (-let ((docstring (when (stringp (car body)) (pop body)))
    ...
    `(prog1 (defun ,name ,args ,docstring ,@body)
       (void:add-advice target ,key it))))
#+end_src

***** TODO get target
:PROPERTIES:
:ID:       524c2a78-828b-4375-a3a6-44058b466393
:END:

#+begin_src emacs-lisp
(defun void:get-target (function)
  "Return the target of FUNCTION."
  (or (void:advice-target fn)
      (void:hook-target fn)))
#+end_src

***** function that return true
:PROPERTIES:
:ID:       d3c14607-2482-44d9-8881-b355434027e5
:END:

#+begin_src emacs-lisp
(defun void:counter (count)
  "Return t when count is zero, otherwise return nil."
  (lambda ()
    (cl-decf count)
    (> count 0)))
#+end_src
...
(defhook! void:increase-garbage-collection-hook ()
  "Boost garbage collection settings to `VOID:GC-CONS-THRESHOLD-MAX'."
  :hook minibuffer-enter-hook
  (setq gc-cons-threshold VOID:GC-CONS-THRESHOLD-MAX))

(defhook! void:restore-garbage-collection-hook ()
...
#+begin_src emacs-lisp
(defhook! void:tangle-on-quit-maybe-hook ()
...
#+begin_src emacs-lisp
(defhook! void:tangle-on-save-hook ()
...
#+begin_src emacs-lisp
(defadvice! void:run-switch-buffer-hooks-advice (orig-fn buffer-or-name &rest args)
...
#+begin_src emacs-lisp
(defadvice! void:run-prev-or-next-buffer-hooks-advice (orig-fn &rest args)
...
#+begin_src emacs-lisp
(defadvice! void:frame-notice-user-settings-advice (orig-fn &rest args)
...
(defhook! exwm:setup-hide-mode-line-hook ()
  :hook exwm-mode-hook
...
#+begin_src emacs-lisp
(defhook! exwm:rename-buffer-to-title-hook ()
...
#+begin_src emacs-lisp
(defhook! exwm:start-in-char-mode-hook ()
...
#+begin_src emacs-lisp
(defhook! company:init-backends-hook ()
...
#+begin_src emacs-lisp
(defhook! company:close-tooltip-hook ()
...
(use-package! org-mu4e
  ...
  (defhook! org-mu4e-render-html-only-once-hook ()
...
#+begin_src emacs-lisp
(defhook! mu4e:wrap-text-in-message-hook ()
...
#+begin_src emacs-lisp
(defhook! smartparens:disable-navigate-skip-match-hook ()
...
#+begin_src emacs-lisp
(defhook! smartparens:init-in-eval-expression-hook ()
...
#+begin_src emacs-lisp
(defhook! smartparens:enable-smartparens-in-minibuffer-hook ()
...
#+begin_src emacs-lisp
(defhook! evil:set-cursor-color-hook ()
  "Set the default cursor shape and size."
  :hook evil-mode-hook
...
#+begin_src emacs-lisp
(defhook! evil:refresh-cursor-hook ()
...
(defhook! evil:enable-insert-state-in-minibuffer-hook ()
  :hook evil-mode-hook
...
(defun evil:save-state-before-minibuffer-hook ()
  "Save state before minibuffer."
  (setq evil:state-before-minibuffer evil-state)
  (evil-insert-state))

(defun evil:restore-state-after-minibuffer-hook ()
...
#+begin_src emacs-lisp
(defhook! eshell:enable-text-wrapping-hook ()
...
#+begin_src emacs-lisp
(defhook! eshell:remove-fringes-hook ()
...
(defhook! eshell:define-aliases-hook ()
...
(defhook! void:unpropertize-kill-ring-hook ()
...
#+begin_src emacs-lisp
(defhook! dired:sort-directories-first-hook ()
...
(use-feature! autorevert
  ...
  :defer-config
  (defhook! auto-revert:revert-buffer-maybe-hook ()
  ...
  (defhook! auto-revert:visible-buffers-h ()
    ...
    (--each (-uniq (mapcar #'window-buffer (window-list)))
      (with-current-buffer it (auto-revert:revert-buffer-maybe-hook)))))
#+end_src
...
#+begin_src emacs-lisp
(defhook! org:goto-first-heading-hook ()
...
#+begin_src emacs-lisp
(defhook! org:ensure-heading-visible-hook ()
...
#+begin_src emacs-lisp
(defadvice! org:ensure-children-visible-advice ()
