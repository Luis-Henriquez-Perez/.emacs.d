...
(defun use-package-normalize/:setq (name keyword args)
  (use-package-as-one () args
    (lambda (label arg)
      (unless (consp arg)
        (use-package-error
         (concat label " a cons cell or a list of cons cells.")))
      arg)))
...
(setq use-package-keywords
      (--splice (eq :init it)
                (list :defer-incrementally it)
                use-package-keywords))

(defun use-package-normalize/:defer-incrementally ()
  (use-package-as-one () args
    (lambda (label arg)
      (unless (symbolp arg)
        (use-package-error
         (concat label " must be a symbol")))
      arg)))

(defun use-package-handler/:defer-incrementally (name _keyword args rest state)
  (use-package-concat
   (use-package-process-keywords name rest state)
