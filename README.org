...
#+begin_src emacs-lisp
(dolist (package '(dash dash-functional anaphora use-package! s))
...
(defmacro use-package! (package &rest args)
  ...
  (declare (indent defun))
  `(use-package ,package
     ,@(thread-last (map-merge 'plist args use-package:defaults)
         (-replace-where #'keywordp #'use-package:true-key))))
#+end_src
...
(defmacro use-feature! (feature &rest args)
  "Wrapper around `use-package!'."
  (declare (indent defun))
  `(use-package! ,feature ,@(plist-put args :ensure nil)))
#+end_src

*** default keywords
:PROPERTIES:
:ID:       3a3e8b96-96ff-4d29-9767-686825a138d5
:END:

#+begin_src emacs-lisp
(defvar use-package:defaults nil
  "Default keywords used for `use-package'.")
#+end_src

*** add alias support
:PROPERTIES:
:ID:       180869b2-db8d-48a6-ac6f-6a5be6313c73
:END:

One thing I miss about leaf is alias support. I would assume that in order to
override a default use-package! keyword, all you'd have to do is override or
alias that keywords normalizer and handler functions. However, it doesn't seem
to be so simple.

**** aliases
:PROPERTIES:
:ID:       f2f69aaa-0641-49d0-8606-172ae5b260b3
:END:

#+begin_src emacs-lisp
(defvar use-package:aliases nil
  "An alist of the form (ALIAS . DEF).
In the `use-package!' macro, keywords named ALIAS will be replaced with DEF.")
#+end_src

**** get-alias
:PROPERTIES:
:ID:       846f7099-131e-45a6-83ad-72fe5cb3c24c
:END:

#+begin_src emacs-lisp
(defun use-package:true-key (key)
  "Return the KEY."
  (or (alist-get key use-package:aliases) key))
#+end_src
...
#+begin_src emacs-lisp
(defun use-package:args-or-error (args valid-p)
  (-let (((args invalid-elt) (use-package:unfold-args args valid-p)))
    (if invalid-elt
        (use-package-error (format "bad elt: %S | list: %S" invalid-elt args))
      args)))
#+end_src
...
**** hook and mode
:PROPERTIES:
:ID:       82ce7194-1619-4ecf-8c0d-d43bbde5f852
:END:
...
(defun use-package:ensure-hook (var)
  (if (s-matches-p "-hook\\'" (symbol-name var))
      var
...
(setq use-package-keywords
      (--splice (eq :straight it) (list :os it) use-package-keywords))
...
#+begin_src emacs-lisp
(setq use-package-keywords
      (--splice (eq :init it) (list :pre-setq it) use-package-keywords))
...
(defun use-package-normalize/:setq (name keyword args)
  (use-package:args-or-error args (lambda (it) (and (consp it) (symbolp (car it))))))
...
#+begin_src emacs-lisp
(setq use-package-keywords
      (--splice (eq :init it) (list :setq-default it) use-package-keywords))
...
(advice-remove 'use-package-normalize/:custom #'use-package-normalize/:custom@return-args-as-is)
(defadvice! return-args-as-is (:override use-package-normalize/:custom)
  <args>)

(advice-remove 'use-package-handler/:custom #'use-package-handler/:custom@use-cons-cell-not-lists)
...
#+begin_src emacs-lisp
(setq use-package-keywords
      (--splice (eq :disabled it) (list it :after-call) use-package-keywords))
...
****** insert
:PROPERTIES:
:ID:       02d4c6fd-7203-4731-aace-17ddd9a52599
:END:

#+begin_src emacs-lisp
(setq use-package-keywords
      (--splice (eq :hook it) (list :vhook it) use-package-keywords))
#+end_src
...
#+begin_src emacs-lisp
(defun use-package-normalize/:vhook (name keyword args)
  (thread-last (use-package:args-or-error args (-orfn #'symbolp #'-cons-pair-p))
...
#+begin_src emacs-lisp
(defun use-package-handler/:vhook (name _keyword args rest state)
  (use-package-concat
   (use-package-process-keywords name rest state)
...
(defun use-package-normalize/:alias (name keyword args)
  (use-package:args-or-error args #'-cons-pair-p))
...
#+begin_src emacs-lisp
(setq use-package-keywords
      (--splice (eq :init it) (list :popup it) use-package-keywords))
...
(use-package! use-package
  ...
  :init
  (setq use-package:defaults (append '(:ensure t) use-package:defaults))
  (push (cons :ensure :straight) use-package:aliases))
#+end_src
...
#+begin_src emacs-lisp
(use-package! paren
  ;; vhook (a-hook . doo) (b-hook . do)
  ;; vhook (prog-mode-hook . show-paren-mode)
  ;; :setq-default
  ;; (show-paren-delay . 0)
  )
#+end_src
...
(use-package! frame
  :straight nil
  :vhook (window-setup . window-divider-mode)
  :custom
...
#+begin_src emacs-lisp
(use-package! exwm
  :vhook (exwm-mode . hide-mode-line-mode)
...
(use-package! exwm-firefox-evil
  :disabled t
  :after exwm
  :vhook (exwm-manage-finish . exwm-firefox-evil-activate-if-firefox)
  :config
  (dolist (k `(escape))
...
(use-package! yasnippet
  :vhook (prog-mode . yas-minor-mode-on)
  :setq
...
(use-package! prescient
  ...
  :vhook (ivy-mode . prescient-persist-mode))
#+end_src
...
#+begin_src emacs-lisp
(use-package! company-prescient :vhook company-mode)
#+end_src
...
(use-package! org-mu4e
  :disabled t
  :vhook (mu4e-compose-mode . org-mu4e-compose-org-mode)
  :setq
...
(defun org/dwim-edit-description ()
  ...
  (-let* ((elements (org:element-headline-elements))
    ...
    (edit-indirect:add-before-commit-hook #'edit-indirect:straight-newline)
...
#+begin_src emacs-lisp
(defun edit-indirect:straight-newline (&optional block-was-empty-p)
...
(defun org/edit-source-block-at-point ()
  ...
  (alet (org-element-context)
    ...
    (-let* (((beg end) (org-src--contents-area it))
            ...
            (edit-indirect-after-creation-hook edit-indirect-after-creation-hook))
      (add-hook 'edit-indirect-after-creation-hook #'edit-indirect:straight-newline)
...
(use-package! smartparens
  :vhook (prog-mode eshell-mode ielm-mode)
  :setq
...
#+begin_src emacs-lisp
(use-package! evil-surround :vhook prog-mode-hook text-mode-hook)
#+end_src
...
#+begin_src emacs-lisp
(use-package! evil-matchit :vhook prog-mode)
#+end_src
...
#+begin_src emacs-lisp
(use-package! all-the-icons-dired :vhook ranger-mode-hook)
#+end_src
...
(use-feature! org-id
  ...
  :vhook (org-insert-heading . org-id-get-create))
#+end_src
...
(use-feature! org-clock
  ...
  :vhook (kill-emacs . org-clock-save)
  :setq
  ;; org-clock-sound
...
(use-feature! org-crypt
  ...
  :vhook (org-reveal-start . org-decrypt-entry)
  ;; :preface
...
(use-package! modern-cpp-font-lock
  :vhook (c++-mode . modern-c++-font-lock-mode))
#+end_src
...
(use-feature! tex
  :mode ("\\.tex\\'" . LaTeX-mode)
  :vhook (LaTex-mode . visual-line-mode)
  :setq
  (TeX-parse-self . t)
  (TeX-auto-save . t)
...
(use-package! adaptive-wrap
  :vhook (LaTeX-mode . adaptive-wrap-prefix-mode)
...
(use-package! dashboard
  :demand t
  :vhook (window-setup . dashboard-insert-startupify-lists)
  :init
...
(use-package! hide-mode-line
  :vhook Man-mode completion-list-mode)
#+end_src
