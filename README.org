...
**** my version of as-one
:PROPERTIES:
:ID:       b4bfce7d-9fa1-4c4f-bf7a-b474e2758f02
:END:

#+begin_src emacs-lisp
(defun use-package:as-one (symbol args fn &optional message)
  (use-package-as-one (symbol-name symbol) args (use-package:check-fn fn message)))
#+end_src
...
#+begin_src emacs-lisp
(defun use-package:check-fn (fn message)
  "Return a function used for `use-package-as-one'."
  `(lambda (label arg)
     (if (not (funcall #',fn arg))
         (use-package-error (format "%s wants %s" label ,message))
       arg)))
#+end_src
...
(setq use-package-keywords
      (use-package-list-insert :os use-package-keywords :ensure))

(defun use-package-normalize/:os (name-symbol keyword args)
  (thread-last "one of the following symbols: mac, windows, linux"
    (use-package:as-one keyword args (-partial #'-contains-p '(mac windows linux)))))
...
#+begin_src emacs-lisp
(setq use-package-keywords
      (use-package-list-insert :pre-setq use-package-keywords :init))
...
#+begin_src emacs-lisp
(setq use-package-keywords
      (use-package-list-insert :setq use-package-keywords :init))
...
(defun use-package-normalize/:setq (name keyword args)
  (let ((message "a cons cell whose CAR is a symbol label")
    ...
    (use-package:as-one keyword args fn message)))
...
#+begin_src emacs-lisp
(setq use-package-keywords
      (use-package-list-insert :setq-default use-package-keywords :init))
...
#+begin_src emacs-lisp
(setq use-package-keywords
      (use-package-list-insert :before-call use-package-keywords :disabled t))

(push :before-call use-package-deferring-keywords)

(defun use-package-normalize/:before-call (name keyword args)
  (use-package-as-one (symbol-name name) args
    (use-package:check-fn #'symbolp "a symbol")))
...
(use-package! use-package
  ...
  ;; (push (cons :ensure :ensure) use-package:aliases)
  )
#+end_src
