...
[[helpfn:doct][doct]] =org-capture-templates= with the appropriate elements added to it based on
the doct declarations. but it does not actually add it. As it is convenient to
declare it and add it in all in one go. Additionally, it removes any capture
templates with the same key. This is so that I can freely re-evaluate it without
cluttering my capture templates with duplicate entries.
