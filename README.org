...
sorted closer to the beginning of the list than keys with a lower number.
...
cons cell. Normalizers allow us to separate.
The reason that the normalizers take a list instead of a value (and then use
that to map through the list) is to deal with nested lists.
