...
#+begin_src emacs-lisp
(defun void:add-hook (hook-var hook-fn &optional depth local expire-fn)
  "Add a VOID hook to add HOOK-VAR."
  (let ((new-name (void:new-advice-name advisor advisee)))
    (defalias new-name advisor)
    (advice-add advisor :after new-name)
    (when expire-fn
      (void:add-expiring-advice hook-fn expire-fn))))
#+end_src
...
(defun void:add-advice (target where advice &optional props expire-fn)
  "Add a VOID advice to add ADVISEE."
...
#+begin_src emacs-lisp
(defun void:add-expiring-advice (target expire-fn &optional unbind)
