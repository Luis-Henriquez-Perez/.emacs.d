...
#+begin_src emacs-lisp
(defun leaf~insert-before (place pair plist)
  "Insert PAIR before PLACE in `leaf-keywords'."
  (-let ((index (-elem-index place leaf-keywords))
         ((key . value) pair))
    (-insert-at index value plist)
    (-insert-at index key plist)))
#+end_src
