...
I'm not stumped or "stuck" with a TODO.
...
Create a pdf file of the page and store it in `VOID-SCREENSHOT-DIR'."
    (let* ((url (exwm::get-url-from-firefox))
      ...
      ;; Does not entirely work yet, at least for certain webpages with unique titles.
      (async-start-process "wkhtmltopdf"
                           "wkhtmltopdf"
                           (lambda (&rest _) (message "DONE"))
                           url
                           (format "%s%s.pdf" VOID-SCREENSHOT-DIR pdf-name))
      (->> (org-ml-build-headline! :level 1 :title-text link-to-pdf)
           (org-ml-headline-set-node-property "ID" (org-id-new))
           (org-ml-headline-set-node-property "CREATED" (ts-format))
  ...
  (org-capture:add-to-templates
   (list "Website"
         ...
         :prepend t
         :empty-lines 1
         :template #'org-capture::website-template)))
#+end_src
