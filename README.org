...
code. Well, not so much =VOID-INIT-ORG-FILE= and =VOID-TODO-FILE= but the others are
referenced alot.
...
(defconst VOID-MAIN-ORG-FILE (concat VOID-EMACS-DIR "README.org")
...
[[helpvar:VOID-INIT-ORG-FILE][void-main-org-file]]. ~void-tangle-on-save-h~ is called whenever a buffer is being
...
(defhook! tangle-maybe (after-save-hook)
  "Tangle `VOID-INIT-ORG-FILE' asynchronously when it is saved."
  (when (and (require 'async nil t)
             (string= (file-truename VOID-INIT-ORG-FILE)
...
(use-feature! org-refile
  :pre-setq
  (org-refile-targets . `((,VOID-INIT-ORG-FILE . (:maxlevel . 10))
