...
Especially when we want to add multiple keyword arguments. The option for adding
keyword arguments before the body of the macro is useful.

#+begin_src emacs-lisp
(defmacro defhook! (action args &rest body)
  "Drop-in replacement for `defhook'.
ACTION should be a verbal phrase that describes what the hook does.
