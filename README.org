...
It is useful to apply some keywords to all emacs packages. An example of this is
ensure.
...
To accommidate different ways users can pass in values to. The reason that the
normalizers take a list instead of a value (and then use that to map through the
list) is to deal with nested lists.
...
This function is meant to be used with [[helpfn:-reduce][-reduce]] to construct the body of the
form. Often there is some metadata that is useful to know when executing these
functions such as the package name (see [[][my definition of :hook]). This
metadata will be included in BINDINGS.

#+begin_src emacs-lisp
(defun def-package-apply-fn (pair forms &optional bindings)
  "Return list of forms from applying VAR to VALUES.
...
Reduce is perfect for creating a form from nil.
...
**** stuff
:PROPERTIES:
:ID:       cd4d3638-a2d9-452e-888a-7934b001822f
:END:
...
Defining a keyword is as simple as writing a function.
