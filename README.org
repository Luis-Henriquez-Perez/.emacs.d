   ...
   (list
    (if-let ((commands (plist-get rest :commands))
        ...
        `(defadvice! ,advice (:before ,@commands)
           (void-log "Loading needed libraries for .")
           ,@body)
      `(after! ,name
         (void-log "Loading needed libraries for .")
         ,@body)))))
...
#+begin_src emacs-lisp
(use-package:insert :system-ensure :commands t)

(defun use-package-normalize/:system-ensure (name keyword args)
  (use-package:args-or-error keyword args #'symbolp "a symbol"))

(defun use-package-handler/:system-ensure (name _keyword features rest state)
  (use-package-concat
   (use-package-process-keywords name rest state)
   (list
    (if-let (commands (plist-get state :commands))
        (--map `(defadvice! ,(intern (format "ensure-%s-for-%s" it name)) (:around ,it)
                  (if (system-packages-package-installed-p it)
                      ;; prompt the user to install the package.
                      (apply <orig-fn> args)
                    (aif (y-or-n-p "%s is needed for %s to work.\nWould you like to install?")
                        (progn (system-packages-install it)
                               (apply <orig-fn> <args>))
                      (message "Aborting the function call of %s." name))))
               ())
      `(after! ,name
