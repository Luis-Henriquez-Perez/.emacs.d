...
(def-package-set-normalizer :hook #'def-package-ensure-cons-cells)
#+end_src
...
#+begin_src emacs-lisp
(def-package-key bind ((state map key def))
  `(after! general
     ,@(def-package-apply :command ',def)
     (general-def ,state ,map ,key ,def)))

(def-package-set-normalizer :bind #'def-package--normalize-bind)

(defun def-package--normalize-bind (list)
  "Normalize."
  (cond (keywordp (car list))
        ())
  list)
#+end_src
