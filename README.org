...
**** process alist
:PROPERTIES:
:ID:       40005257-0bd8-4e5a-b422-ae2df3931028
:END:

#+begin_src emacs-lisp
(defvar leaf~process-alist nil)
#+end_src
...
Processing functions should take in a key and other values and they should
return an alist of key value.

#+begin_src emacs-lisp
(defun leaf~general-process (key value)
  "Return a cons cell with key as cdr."
  (if (and (atom value) (not (-cons-pair-p value)))
      (list (cons key value))
    (-annotate (-const key)
               (if (= (length value) 1) (car value) value))))

(push (cons t #'leaf~general-process) leaf~process-alist)
#+end_src

**** preprocess fn
:PROPERTIES:
:ID:       16bf5a3b-4357-4c27-bd21-baebbb40ef2f
:END:

#+begin_src emacs-lisp
(defun leaf~pre-process-fn (pair)
  (alet (or (alist-get (car pair) leaf~process-alist)
            (alist-get t leaf~process-alist))
    (funcall it (car pair) (cdr pair))))
#+end_src

**** bind key preprocess
:PROPERTIES:
:ID:       25939414-7f07-4ea3-9354-c92ae3bff2a0
:END:
