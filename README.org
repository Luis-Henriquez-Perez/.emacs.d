...
** keybindings
:PROPERTIES:
:ID:       5059b055-493f-4ba1-9738-3fa752308ddf
:END:

*** with-keymap!
:PROPERTIES:
:ID:       76351d85-1d89-4057-b82e-dc6cf539f124
:END:

#+begin_src emacs-lisp
(defmacro with-keymap (keymap &rest body)
  (if (keymapp keymap)
      (progn ,@body)
    (defhook! load-bindings ()
      "Load"
      (progn ,@body))))
#+end_src
...
*** kbd!
:PROPERTIES:
:ID:       8607e884-34f1-436b-9110-c2ad50571203
:END:
...
*** define-key!
:PROPERTIES:
:ID:       9729c413-84bd-44b9-8be3-959adb212498
:END:

#+begin_src emacs-lisp
(defmacro define-key! (states keymaps &rest bindings)
  "Define keybindings."
  (let ((define-key (if states 'evil-define-key 'define-key)))
    `(progn
       ,@(->> (-partition 2 bindings)
              (-table-flat #'list (-list states) (-list keymaps))
              (-map (-lambda ((state map (key value)))
                      `(,define-key ',state ,map (kbd! ,key) ,value)))))))
#+end_src
