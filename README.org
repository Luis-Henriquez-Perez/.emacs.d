...
(defun use-package:args-as-one (args &optional valid-item-p)
  (if (-all-p valid-item-p args)
      args
    (if (alet (car args)
          (and (listp it) (not (-cons-pair-p it))
               (= 1 (length it))))
        (use-package:args-as-one (car args) valid-item-p)
      (use-package-error "Invalid argument: %s" (-find (-not valid-item-p) args)))))
#+end_src
