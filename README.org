...
ADVICE is an advice of the form \"advisee@advisor\"."
  (->> (symbol-name advice)
...
Note that this function must be evaluated with lexical binding to work. Also
note that in the case where an expire advise is
...
#+begin_src emacs-lisp
(defadvice! replace-hook-done-message (:override defhook-done)
  "Set `defhook-done' message to \"add FN to HOOK\"."
  (void:log "add %s to %s" <func-sym> <hook-sym>))
#+end_src
