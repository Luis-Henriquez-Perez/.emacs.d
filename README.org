...
This package saves buffers during idle time and after certain commands. The idea
is that we want to avoid suffering a pause or lag when we're actually doing
something. So we save only when its likely not to interfere with our workflow.
