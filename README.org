...
even other [[https://github.com/caisah/emacs.dz][emacs configs]], screenshots are underutilized or, more often, not
utilized at all. In Void, where documentation is *very* important, I supplement
my descriptions with pictures so others have a better idea of what I'm taking
about.
...
[[https://github.com/emacsmirror/screenshot][screenshot]] is another emacs screenshot package that. It doesn't have an
option for.
...
(use-package! screenshot
  :commands screenshot
  :setq
  (screenshot-take-delay . 0)
  (screenshot-schemes . `(("local" :dir ,VOID:SCREENSHOT-DIR))))
#+end_src
...
This package doesn't see exwm application windows. You just get a [[file:./screenshots/black-window-on-top-should-be-iceweasel.png][black window]]
where the application should go.

#+begin_src emacs-lisp
(use-package! escr
  :ensure (:host github :repo "atykhonov/escr")
  :commands (escr-window-screenshot escr-frame-screenshot)
  :setq
  (escr-screenshot-directory . (concat VOID:EMACS-DIR "multimedia/screenshots")))

(general-def
  "<f1>" #'escr-window-screenshot
  "<f2>" #'escr-frame-screenshot)
#+end_src
...
Scrot has this wierd quirk where it tries to upload the screenshots.

After I use scrot exwm windows don't show anymore.
...
(use-package! scrot
  ...
  (scrot-args . "--focused")
  (scrot-file-ext . "png")
  (scrot-local-path . (concat VOID:EMACS-DIR "multimedia/screenshots")))

(general-def "<f5>" #'scrot)
;; prevent scrot from uploading stuff
(advice-add #'scrot-upload :override #'ignore)
#+end_src
...
(use-package! gif-screencast
  :setq
  (gif-screencast-screenshot-directory . VOID:SCREENSHOT-DIR))

(general-def "<f6>" #'gif-screencast-start-or-stop)
#+end_src
