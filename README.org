...
Like =use-package=, leaf is a macro that facilitates package configuration. It's
...
#+begin_src emacs-lisp
(defun leaf~insert (plist place key value &optional after)
  "Return PLIST with KEY and VALUE inserted before PLACE."
  (alet (+ (if after 2 0) (-elem-index place plist))
    (thread-last plist
      (-insert-at it value)
      (-insert-at it key))))
#+end_src
...
*** remove keyword
:PROPERTIES:
:ID:       1f00e14e-8e1f-425e-b245-f22d52af9f33
:END:

#+begin_src emacs-lisp
(defun leaf~remove (plist key)
  "Remove KEY and its corresponding value from PLIST."
  (aif (-elem-index key plist)
      (-remove-at-indices (list it (1+ it)) plist)
    plist))
#+end_src
...
(defun leaf~anaphoric-vars ()
  "Return anaphoric variants of the leaf variables."
...
This macro is meant to make defining a leaf keyword extremely easy. Like
[[id:1e0f3a27-a7d8-4e28-a359-f42ed7a16033][defadvice!]] and [[id:4daf2baf-ea7f-41f5-9f86-63168089149a][defhook!]], the syntax is designed to resemble [[helpfn:defun][defun]]. I'd also like
to allow for the re-evaluation of this macro form to update a leaf keyword.
...
(defmacro defleaf! (name args &rest body)
  ...
  (-let* ((wrap-p (s-suffix-p "*" (symbol-name name)))
          ...
          (leaf-apply (void-symbol-intern 'leaf~~apply- name)))
    `(progn
       (setq leaf-keywords (leaf~remove leaf-keywords ,keyword))
       (setq leaf-keywords
             (leaf~insert leaf-keywords
                          ,(void-keyword-intern place)
                          ,keyword
                          '(,leaf-apply ,@(leaf~vars))
                          ,(eq where 'after)))
       (defun ,leaf-apply (,@(leaf~anaphoric-vars))
         (-let ((,lambda-args (-list <value>)))
           ,(if wrap-p `(list (progn ,@body))
              `(cons (progn ,@body) (macroexp-progn <body>))))))))
#+end_src
...
(defleaf! void-hook (before (var . hook) hook)
  (leaf-register-autoload hook <name>)
  (alet (or hook (void-symbol-intern <package> '-mode))
    `(void-add-hook ',var #',it)))
#+end_src
...
#+begin_src emacs-lisp
(defleaf! void-setq (before (var . value) setq)
  `(setq ,var ,value))
#+end_src
...
(defleaf! before-call (before (fn) hook)
  (leaf-register-autoload fn <package>)
