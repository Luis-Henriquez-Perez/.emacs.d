...
#+begin_src emacs-lisp
(defun exwm::get-url-from-qutebrowser ()
  (interactive)
  (let ((fid (exwm--buffer->id (current-buffer))))
    (sleep-for 0 300)
    ...
    (aprog1 (current-kill 0)
      (message "Copied %S to the kill ring." it))))
#+end_src
...
#+begin_src emacs-lisp
(defun browse-url-qutebrowser (url &optional _)
...
(use-package! org-pretty-tags
  ...
  (push (cons "test" (all-the-icons-fileicon "test-generic"))
        org-pretty-tags-surrogate-strings)
...
(defmacro define-capture-template! (name args &rest body)
  ...
  (let* ((string-name (downcase (symbol-name name)))
         (node-fn (intern (format "org-capture::%s-template-node" string-name)))
         (string-fn (intern (format "org-capture::%s-template-string" string-name)))
         (body (or body '((org-capture:default-template))))
         (defaults org-capture:defaults)
         (key (downcase (substring string-name 0 1))))
    `(after! org-capture
       (defun ,node-fn ()
         "Return capture template node for."
         ,@body)
       (defun ,string-fn ()
         "Return capture template as a string."
         (org-ml-to-trimmed-string (,node-fn)))
       (org-capture:add-to-templates
        (cons ,(symbol-name name)
              (-concat ,args org-capture:defaults '(:keys ,key :template ,string-fn)))))))
#+end_src
...
The org-capture file is the default place for all capture templates.

#+begin_src emacs-lisp
(defun org-capture:file ()
  "Return the org capture file."
  VOID-CAPTURE-FILE)
#+end_src
...
***** bug
:PROPERTIES:
:ID:       95c8bc05-0417-4f6e-953b-6df6ef5bbada
:END:

#+begin_src emacs-lisp
(define-capture-template! Bug ()
  (->> (org-capture::generic-template)
       (org-ml-set-property :todo-keyword "BUG")))
#+end_src
...
up. These are capture templates to add the url of the current webpage I'm
visiting into an org file as a level one heading. This is like my own
"bookmarks"/"history" for sites I fond useful or interesting.

****** website-buffer
:PROPERTIES:
:ID:       7bbe8115-0c99-4c79-ba39-5ce34216285d
:END:

#+begin_src emacs-lisp
(defun org-capture::browser-buffer-p ()
  "Return whether a buffer is an browser buffer."
  t)
#+end_src
...
Assumes the url is at the top of the kill ring."
  (let* ((program "wkhtmltopdf")
         ...
         (webpage-title (s-replace "/" "~" (exwm::webpage-name exwm-title)))
...
#+begin_src emacs-lisp
(define-capture-template! Website (:immediate-finish t
                                   :after-finalize #'org-capture::save-webpage)
  (let* ((url (exwm::browser-url))
         (pdf-name (exwm::browser-webpage-name))
...
#+begin_src emacs-lisp
(use-package! org-agenda :straight nil :after org :commands
