...
#+begin_src emacs-lisp
(defvar void:function-name-regexp nil
...
#+begin_src emacs-lisp
(defvar hook-fn-regexp (rx (group (1+ anything)) "%" (group (1+ anything)))
...
#+begin_src emacs-lisp
(defun void:new-hook-name (hook-var hook-fn)
  "Return new hook name based on HOOK-VAR and HOOK-FN."
  (->> (s-match hook-fn-regexp hook-fn)
       (nth 1)
       (void:symbol-intern hook-var "%")))
#+end_src

***** TODO void get hook var
:PROPERTIES:
...
(defun void:hook-var (hook-fn)
  "Return the variable that hook-fn is attached to."
  (->> (symbol-name hook-fn)
       ()))
#+end_src
...
#+begin_src emacs-lisp
(defvar void:advice-fn-regexp (rx (group (1+ anything)) "@" (group (1+ anything)))
