...
**** leaf~insert-form
:PROPERTIES:
:ID:       02313c96-46f1-45ae-97aa-2a9e66e8e572
:END:

#+begin_src emacs-lisp
(defun leaf~define-keyword (keyword where place form &optional aliases defer normalizer)
  ...
  (when normalizer
    (leaf~set-normalizer normalizer)))
#+end_src
...
(defmacro defleaf! (name args &rest body)
  ...
  (-let (((where place alias defer normalizer) args)
    ...
    `(progn
       ...
       ,@(->> (-list alias)
              (-map #'void-keyword-intern)
              (--map `(push (cons ,it ,name) leaf-alias-keyword-alist)))

       ,@(whenlist! defer
           `(push ,keyword leaf-defer-keywords))

       ,@(whenlist! normalizer
           (leaf:set-normalizer ,keyword ,normalizer))
