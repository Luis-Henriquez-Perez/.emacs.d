...
- Extensive, lightweight profiling of each file loaded
- Gracefully error in initialization files and log
...
:END:
  The file structure of my configuration is designed to be simple.
  Other than the =init.el=, all files that are loaded immediately are named by a
  three-digit number to reflect the order in which they are loaded.  Doing so
  allows me to be able to programmatically load files in bulk without having to
  specify a specific load order.  Additionally, this makes how my configuration
  works more transparent in that someone looking at my files immediately knows
  what is loaded in what order.  In the file names blow =xxx= refers to a three
  digit prefix.
