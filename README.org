...
(defpackage-set-normalizer :after #'def-package-ensure-form-list)

(push (cons :after #'def-package-ensure-form-list) def-package~normalizer-alist)
#+end_src
...
(def-package-set-weight :disabled 25)
#+end_src
...
(push (cons :alias #'def-package-ensure-cons-cells) def-package~normalizer-alist)
#+end_src
...
***** TODO exec path from shell                                       :disabled:
:PROPERTIES:
