...
#+begin_src emacs-lisp
(defun leaf~insert-after (list elem targetlist)
  (let ((place (+ 2 (-elem-index elem list))))
    ;; (cl-assert (not (keywordp (elt list (1- place)))))
    (--each targetlist
      (setq list (-insert-at place it list))
      (++ place))
    list))
#+end_src
...
#+begin_src emacs-lisp
(defun leaf~insert-at (lst alem targetlst)
