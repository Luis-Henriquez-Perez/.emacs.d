...
(defadvice! abbrev-hooks-and-modes (:override use-package-normalize/:hook)
  (message "NORMALIZE ARGS: %S" <args>)
  (let ((args (--map-when (symbolp it) (cons it nil) <args>)))
    (-zip-with #'cons
               (--map-when (not (s-matches-p "-hook\\'" (symbol-name it)))
                           (void-symbol-intern it "-hook")
                           (-map #'car args))
               (--map-when (null it)
                           (void-symbol-intern <name> "-mode")
                           (-map #'cdr args)))))
#+end_src
...
(defadvice! use-void-add-hook (:override use-package-handler/:hook)
  (message "HANDLE ARGS: %S" <args>)
  (use-package-concat
   (use-package-process-keywords <name> <rest> <state>)
   (mapcar (-lambda ((sym . fun)) `(void-add-hook ',sym #',fun))
           <args>)))
#+end_src
