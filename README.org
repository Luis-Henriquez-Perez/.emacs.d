...
(defun use-package-normalize/:setq (name keyword args)
  (let ((check (lambda (it) (and (consp it) (symbol-p (car it)))))
        )
    (use-package:args-as-one check args message))
  )
...
#+begin_src emacs-lisp
(defun use-package:args-as-one (args &optional valid-item-p)
  (aif (or (--find (not (funcall valid-item-p it)) args)
           (when (and (listp (car args))
                      (not (-cons-pair-p (car args)))
                      (= 1 (length (car args))))
             (--find (not (funcall valid-item-p it)) (car args))))
      (use-package-error (format "Invalid argument %s." it))
    args))
#+end_src
