...
#+begin_src emacs-lisp
(defun void:advice-target (advice)
  "Return the function ADVICE advises.
...
The main question here was how should the arguments be ordered. The advice first
or the target? In the end I choose to be consistent the order used with
[[helpfn:advice-add][advice-add]].

#+begin_src emacs-lisp
(defun void:advice-name (target advice)
  "Return advice name that meets Void naming conventions.
Advice name is of the form \"target@advice\"."
  (intern (format "%s@%s" target advice)))
#+end_src
...
(defun void:add-advice (target where advice &optional props expire-fn)
  ...
  (let ((name (void:advice-name target advice)))
    (defalias name advice)
    (advice-add target where name props)
    (when expire-fn
      (advice-add it :around (void:expire-advice target name)))))
#+end_src

***** removing advice
:PROPERTIES:
:ID:       baf2dd59-b37b-47bc-a1d2-815ba925d1bd
:END:

This is counter part of [[id:4750f4dc-053b-4062-bd6c-aeeed6cdbcd9][adding advice]]. It provides convenience for removing an
advice.

#+begin_src emacs-lisp
(defun void:remove-advice (target advice)
  "Remove Void ADVICE from TARGET."
  (advice-remove target (void:advice-name target advice)))
#+end_src
...
=expire-advice=. An =expire-advice= is meant to be attached to a void advice and.

Note that this function must be evaluated with lexical binding to work.

#+begin_src emacs-lisp
(defun void:expire-advice (target &optional expire-fn unbind)
  "Return an advice that causes TARGET to expire when EXPIRE-FN returns true.
TARGET is a function. EXPIRE-FN is a function that returns true when TARGET
should expire."
  (let ((expire-advice (void:advice-name target 'expire))
        (expire-fn (or expire-fn (lambda () t))))
    (fset expire-advice
          (lambda (orig-fn &rest args)
            (aprog1 (apply orig-fn args)
              (when (funcall expire-fn)
                (when (void:advice-p target)
                  (advice-remove (void:advice-target target) target))
...
****** helper
:PROPERTIES:
:ID:       9899e994-4594-451a-857e-ed12168e8b12
:END:

This helper function contains the logic for how the arglist and the body should
look in =defadvice!=. I separated this out into a function so I could keep the
body of =defadvice!= readable and neat.

The most interesting case is what happens when the user doesn't specify
arguments. In that case, you get the advised functions arguments using
[[helpfn:help-function-arglist][help-function-arglist]] and bind them with [[helpfn:cl-progv][cl-progv]]--all this is done when the
function is run.

#+begin_src emacs-lisp
(defun void::defadvice-info (where target args body)
  "Return the appropriate body and arglist for `defadvice!'.
If ARGS is non-nil, return (ARGS BODY). Otherwise, if WHERE is :around"
  (cond ((not (eq 'none args))
         (list args body))
        ((eq where :around)
         (list '(<orig-fn> &rest <args>) body))
        (t
         (let ((args '<args>))
           `((&rest ,args)
...
The =defadvice!= macro is a wrapper around =define-advice= with some notable
differences.

First, unlike [[helpfn:define-advice][define-advice]], the symbol used for naming the advice (the
ACTION argument in =defadvice!=) first instead of the name of the advised
function. Since this symbol is what distinguishes this new advice, I feel that
it should be the it should be the focus of this syntax.

Second, I allow =defadvice!= to accept multiple advised functions since advising
more than one function happens often. Note, however, in cases where you want to
define an advise for more than two or three functions it is preferable not to
use =defadvice!= so as not to deal with really long lines (this is the same reason
for not having functions with too many arguments).

Finally, you can omit the arguments of the advice. If you do, you can use the
arguments of the original function you can do so with [[id:9938b1e1-6c6e-4a45-a85e-1a7f2d0bf6df][anaphoric symbols]]. For an
around advice the arguments become =(<orig-fn &rest <args>)=. For any other advice
the arguments are anaphoric versions of the advised function's arguments. For
examples, see [[id:][the changelog]].

#+begin_src emacs-lisp
(defmacro defadvice! (action args &rest body)
  "A replacement for `define-advice'.
ACTION should be a symbol denoting what the advise is doing.
WHERE is the advice keyword. If ADVICE-ARGS is the argument list for the advice.
TARGET is the name of the function to advise. TARGETS are other functions to be
advised.

If ADVICE-ARGS is ommitted you can refer to the advised function's arguments via
anaphoric symbols.

If `void:debug-p' is non-nil, this function output the ACTION.
...
\(fn ACTION (WHERE &optional ADVICE-ARGS TARGET &rest TARGETS) &rest BODY)"
  ...
  (-let* ((args (if (not (listp (nth 1 args)))
                    (-insert-at 1 'none args)
                  args))
          ((where advice-args target . targets) args)
          (docstring (if (stringp (car body)) (pop body) nil))
          ((arglist body) (void::defadvice-info where target advice-args body)))
    (when target
      `(progn
         (define-advice ,target (,where ,arglist ,action)
           ,docstring
           (void:log "%s @ %s" #',target ',action)
           ,@body)
         (defadvice! ,action (,where ,@targets)
           ,docstring
           ,@body)))))
#+end_src
...
I don't need messages about the.

#+begin_src emacs-lisp
(void:add-advice #'defhook-done :override #'ignore)
#+end_src
...
#+begin_src emacs-lisp
(defconst VOID:ADVICE-KEYWORDS
  '((,(rx "defadvice!" (1+ white) (group (1+ (not white))))
     (1 font-lock-keyword-face)
     (2 font-lock-function-name-face))))

(defhook! add-defadvice-keywords (emacs-lisp-mode-hook)
  "Add `defadvice' keywords to `emacs-lisp-mode'."
  (font-lock-add-keywords nil VOID:ADVICE-KEYWORDS))
#+end_src
...
(unless (display-graphic-p)
  ...
  (defhook! init-tty (window-setup-hook)
    (void:remove-advice #'tty-run-terminal-initialization #'ignore)
...
#+begin_src emacs-lisp
(defadvice! run-after-switch-buffer-hook (:around switch-to-buffer display-buffer)
  "Run `void:after-switch-buffer-hook'."
  (let ((gc-cons-threshold most-positive-fixnum)
        (buffer-or-name (car <args>))
        (args (cdr <args>)))
...
#+begin_src emacs-lisp
(defadvice! run-prev-or-next-buffer-hook (:around switch-to-next-buffer switch-to-previous-buffer)
  (let ((gc-cons-threshold most-positive-fixnum))
...
**** set x-font
:PROPERTIES:
:ID:       b7269613-d859-4d23-8a14-d8fb5f845ec8
:END:

#+begin_src emacs-lisp
(defun void/set-x-font ()
  "Set the x-font for the current frame."
  (interactive)
  (set-frame-font (completing-read "Select font: " (x-list-fonts "*"))))
#+end_src
...
(use-package! auto-yasnippet
  :defer-config (void:add-advice #'aya-expand :after #'evil-insert-state)
...
=counsel-org-goto= is lets you fuzzy search a term among org headings. Note that I
get an error when after advising [[][]].

#+begin_src emacs-lisp
(defadvice! org:prettify-nearby-headlines (:around counsel-org-goto-action)
