...
file being saved. This package saves buffers during idle time and after certain
commands like [[helpfn:switch-to-buffer][switch-to-buffer]] (see [[helpvar:super-save-triggers][super-save-triggers]]). So we save only when
its likely not to interfere with our workflow.
