...
**** template syntax
:PROPERTIES:
:ID:       ebb31724-10cc-4598-936c-8075076d520e
:END:
...
***** add to templates
:PROPERTIES:
:ID:       4855cc10-a3ed-4ad1-90cf-4455b267346c
:END:
...
***** generate the template with org-ml
:PROPERTIES:
:ID:       c1a9c9ee-be52-48d0-b069-796a02972bdc
:END:

Using org-ml guarantees that my headline will be the correct format.

#+begin_src emacs-lisp
(defun org-capture:generic-template ()
  "Generate generic capture template."
  (-let* ((id (org-id-new))
          (date-created (format-time-string "%02m/%02d/%02Y at HH:MM"))
          (start-date)
          (end-date)
          (planning-node (org-ml-build-planning! :scheduled start-date
                                                 :deadline end-date)))
    (->> (org-ml-build-headline! :title-text "%?"
                                 :todo-keyword "TODO"
                                 :tags '("emacs"))
         (org-ml-headline-set-node-property "ID" id)
         (org-ml-headline-set-node-property "CREATED" date-created)
         (org-ml-headline-set-planning planning-node)
...
(after! org-capture
  (doct (list "Emacs"
              ...
              :prepend t
              :empty-lines 1
              :template #'org-capture:generic-template)))
#+end_src
