...
these. To accommidate different ways users can pass in values to. The reason
that the normalizers take a list instead of a value (and then use that to map
through the list) is to deal with nested lists.
