...
(defadvice! abbrev-hooks-and-modes (:override #'use-package-normalize/:hook)
  (thread-last (use-package:args-as-one <args>)
    ...
    (map-values-apply (-partial #'use-package:ensure-mode name))))

(defun use-package:ensure-mode (val package)
  (or val
      (void-symbol-intern <name> (unless (string-match-p )))
      ))
(lambda (var)
  (if (s-matches-p (rx "-hook" eos))
      var
    (void-symbol-intern var "-hook")))#+end_src
