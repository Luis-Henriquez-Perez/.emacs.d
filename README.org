...
- *fast startup*
  ...
  second.  I currently have a startup time of =1.31= seconds (this is with
  byte-compilation).
- *performant*
...
:END:
- Streamlined byte-compiling and testing with [[https://github.com/emacs-eldev/eldev][Eldev]]
...
- Asynchronous package management and installation via [[https://github.com/progfolio][elpaca]]
# - Custom command switch specifically for initial package install
- Automated startup file loading with [[file:lisp/002-init-loader.el][require! macro]]
- Startup time is about =1.46= seconds with byte-compilation
- Complete reproducibility using =elpaca= lockfile features
- No compilation warnings or errors
- Optimized for speed by evaluating macros during compilation
- Extensive abbrev configuration with well over [[file:lisp/999-plain-text-abbrevs.el][1300 abbrevs]]
...
- Beautiful, [[file:lisp/123-base-mode-line.el][hand-crafted modeline]] with [[https://github.com/milkypostman/powerline][powerline]] separators
...
:END:
  The way that I named and organized my files is deliberate and a direct
  reflection of my goals and reasoning.  I separate most code into "init", forms
  that are evaluated immediately, and "config" , forms that will be evaluated
  after some feature is loaded.  This division lends itself to the the deferred
  loading of features while also allowing me to write declarative code in each
  file as opposed to code encased with [[/home/luis/.config/documentation/29.4-helpful--eval-after-load.html][eval-after-load]].  Other than the =init.el=,
  all files that are loaded immediately are named by a three-digit number to
  reflect the order in which they are loaded.  A numeric prefix virtually
  guarantees that file names will not conflict with the name of a built-in or
  external feature, greatly simplifies the task of programmatically loading my
  files and provides a level of transparency to any reader, ensuring that they
  know the precise order my files at a glance.
