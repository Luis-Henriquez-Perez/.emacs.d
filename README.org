...
#+begin_src emacs-lisp
(defconst VOID-ADVICE-SEPARATOR "@"
  "Separator used when naming advice.")

(defconst VOID-HOOK-SEPARATOR "&"
  "Separator used when naming hooks.")
#+end_src
...
#+begin_src emacs-lisp
(defun exwm::get-url-from-firefox ()
...
(defcommand! take-screenshot ()
  ...
  (let* ((entered-name (read-string "Image Name: "))
         ...
         (filename (format "%s%s.svg" VOID-SCREENSHOT-DIR image-name))
...
(after! org
  (setq org-todo-keywords
        '((sequence "TODO" "PROJ" "NEXT" "WAITING" "|" "DONE")
          (sequence "|" "DUPLICATE")
          (sequence "|" "CANCELLED"))))
#+end_src
...
**** org pretty tags
:PROPERTIES:
:ID:       e74e6fc4-0f6c-4c48-b6a2-3a31f82a9078
:END:

This package displays tags as images instead of as words. This has the advantage
of making tags quicker to read at a glance.

#+begin_src emacs-lisp
(use-package! org-pretty-tags
  :commands org-pretty-tags-global-mode
  :config
  (push (cons "idea" (all-the-icons-octicon "light-bulb"))
        org-pretty-tags-surrogate-strings)

  (push (cons "git" (all-the-icons-alltheicon "git"))
        org-pretty-tags-surrogate-strings)

  (push (cons "emacs" (all-the-icons-fileicon "emacs"))
        org-pretty-tags-surrogate-strings)

  (push (cons "cpp" (all-the-icons-alltheicon "cplusplus"))
        org-pretty-tags-surrogate-strings)

  (push (cons "font" (all-the-icons-faicon "font"))
        org-pretty-tags-surrogate-strings)

  (push (cons "org" (all-the-icons-fileicon "org"))
        org-pretty-tags-surrogate-strings)

  (push (cons "payment" (all-the-icons-faicon "money"))
        org-pretty-tags-surrogate-strings)

  (push (cons "school" (all-the-icons-material "school"))
        org-pretty-tags-surrogate-strings)
...
(after! org-capture
  (defun org-capture:add-to-templates (declarations)
    "Convert DECLARATIONS add the result to `org-capture-templates'.
                       ...
                       (when templates
                         (cons (car templates)
                               (--remove (string= (caar templates) (car it))
                                         (clean (cdr templates)))))))
      (setq org-capture-templates (clean (doct declarations))))))
#+end_src
...
For capture templates that have deadlines, I set a default start and end date.

#+begin_src emacs-lisp
(after! org-capture
  (defun org-capture::planning-node (days-difference)
    "Return the planning time."
    (let* ((now (ts-now))
...
(defun org-capture::generic-template ()
  "Return the capture template for Emacs TODOs (as a string)."
...
I use [[][org-ml]] and [[][doct]] to generate templates dynamically. By "dynamically"
I mean that a template for a given key is different every time I open it. This
is possible with regular capture templates via [[][]], but I think the
...
(after! org-capture
  (defun org-capture::emacs-template ()
    "Return the capture template for Emacs TODOs as a node"
    (->> (org-capture::generic-template)
         (org-ml-set-property :todo-keyword "TODO")
         (org-ml-set-property :tags '("emacs"))
         (org-ml-headline-set-planning (org-capture::planning-node 7))))

  (org-capture:add-to-templates
   (list "Emacs"
         :keys "e"
         :file (-const (concat VOID-ORG-DIR "todo.org"))
         :prepend t
         :empty-lines 1
         :template (-compose #'org-ml-to-trimmed-string
                             #'org-capture::emacs-template))))
#+end_src
...
A =TODO= is just for a task that I need to do later. The purpose of this template
is for problems and issues I'm stumped on. It's similar to a TODO except that
I'm not stumped or "stuck" with a TODO. Here I write the description of the
problem and what exactly I want as a solution. This process can help me reason
through as much as I can and provide the perfect draft for a potential question
for reddit or stackexchange.

#+begin_src emacs-lisp
(after! org-capture
  (defun org-capture::problem-template ()
    "Return the capture template for Emacs TODOs (as a string)."
    (org-capture::generic-template))
  ...
  (org-capture:add-to-templates
   (list "Problem"
         :keys "p"
         :file (-const (concat VOID-ORG-DIR "problems.org"))
         :prepend t
         :empty-lines 1
         :template (-compose #'org-ml-to-trimmed-string
                             #'org-capture::generic-template))))
...
(after! org-capture
  ...
  (org-capture:add-to-templates
   (list "Notes"
         :keys "n"
         :file (-const (concat VOID-ORG-DIR "notes.org"))
         :prepend t
         :empty-lines 1
         :template (-compose #'org-ml-to-trimmed-string
                             #'org-capture::generic-template))))
...
***** browser
:PROPERTIES:
:ID:       f265705a-d8c1-49cc-967a-67ebd700d5a9
:END:

These are capture templates to.
...
Saving the webpage using wkhtmltopdf.

#+begin_src emacs-lisp
(defun org-capture::save-webpage ()
  "Save the webpage.
Assumes the url is at the top of the kill ring."
  (let* ((program "wkhtmltopdf")
         (name (format "%s - %s" program (ts-format)))
         (title-regexp "\\([^z-a]+\\) - \\(?:Firefox\\|Ice\\(?:cat\\|weasel\\)\\)")
         (webpage-title (thread-last (s-match title-regexp exwm-title)
                          (nth 1)
                          (s-replace "/" "~")))
         (pdf-path (format "%s%s.pdf" VOID-SCREENSHOT-DIR (s-replace "/" "~" webpage-title)))
         (url (car kill-ring))
         (fn `(lambda (&rest _)
                (if (file-exists-p ,pdf-path)
                    (message "Webpage saved.")
                  (warn "Failed to save webpage %s to %s." ,url ,pdf-path)))))
    (message "%s <-- %s" (f-abbrev pdf-path) url)
    (async-start-process name program fn url pdf-path)))
#+end_src
...
While doing research for solving a problem I tend to browse many, many sites.
Often after browsing I think: "Oh, that site I visited could be useful". But I
can't easily find it again. I don't remember the exact search terms I used.
Either I painstakingly check my browser history one at a time, or I just give
up. This capture template is designed to solve this problem by allowing me to
store the link to any webpage I find important.
...
(after! org-capture
  (defun org-capture::website-template ()
    "Return the capture template websites when used with exwm.
Create a pdf file of the page and store it in `VOID-SCREENSHOT-DIR'."
    (let* ((url (exwm::get-url-from-firefox))
           (title-regexp "\\([^z-a]+\\) - \\(?:Firefox\\|Ice\\(?:cat\\|weasel\\)\\)")
           (pdf-name (nth 1 (s-match title-regexp exwm-title)))
  ...
  (org-capture:add-to-templates
   (list "Website"
         :keys "w"
         :contexts '(:in-buffer "[^z-a]+ - \\(?:firefox\\|ice\\(?:cat\\|weasel\\)\\)\\'"
                     :when (and (featurep 'exwm)
                            (-any-p #'executable-find '("firefox" "iceweasel" "icecat"))))
         :file (-const (concat VOID-ORG-DIR "websites.org"))
         :prepend t
         :empty-lines 1
         :immediate-finish t
         :after-finalize #'org-capture::save-webpage
         :template (-compose #'org-ml-to-trimmed-string
                             #'org-capture::website-template))))
#+end_src
...
**** interactive agenda command
:PROPERTIES:
:ID:       d6e7f76d-2084-4620-bb9d-ea1c2e7fb785
:END:

I don't care much for the other org agenda options, I just want to see the
agenda itself tyvm.

#+begin_src emacs-lisp
(defun org-agenda/agenda ()
  (interactive)
  (org-agenda nil "a"))
#+end_src
...
** evil collection
:PROPERTIES:
:ID:       a89e4935-6a3d-4c6e-9918-6db03ec2d523
:END:
...
** package-specific
:PROPERTIES:
:ID:       a1b87a13-d248-4247-bbbc-04667a477001
:END:
