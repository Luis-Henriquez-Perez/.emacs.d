...
(defun org-capture::save-webpage ()
  ...
  (let* ((program "wkhtmltopdf")
         ...
         (webpage-title (nth 1 (s-match title-regexp exwm-title)))
         (pdf-path (concat VOID-SCREENSHOT-DIR (s-replace "/" "~" webpage-title)))
         (url (exwm::get-url-from-firefox))
         (fn `(lambda (&rest _)
                (if (file-exists-p ,pdf-path)
                    (message "Added page.")
                  (warn "%s NOT added." ,pdf-path))))))
...
***** code
:PROPERTIES:
:ID:       2f4c41be-8974-4972-9a03-6a8d8c6e85f4
:END:

This is for capture templates that have no real place.
