...
(defun use-package-normalize/:pre-setq (name keyword args)
  args)
...
(defun use-package-normalize/:setq-default (name keyword args)
  args)
...
#+begin_src emacs-lisp
(setq use-package-keywords (-remove-item :custom use-package-keywords))
...
(defadvice! abbrev-hooks-and-modes (:override use-package-normalize/:hook)
  (alet (--map-when (not (-cons-pair-p it)) (cons it nil) <args>)
    ())
  (thread-last <args>

    (--map-when (not (s-matches-p "-hook\\'" (symbol-name it)))
                (void-symbol-intern it "-hook"))
    (map-keys-apply #'use-package:ensure-hook)
    (map-values-apply (-partial #'use-package:ensure-mode))))

(defun use-package:ensure-hook (hook)
  (if (not (s-matches-p (rx "-hook" eos) (symbol-name hook)))
      (void-symbol-intern hook '-hook)
    hook))

(defun use-package:ensure-mode (package name)
  (or name
      ()))
...
(defadvice! use-void-add-hook (:override use-package-handler/:hook)
  (use-package-concat
   ...
   ;; (cl-mapcan
   ;;  (-lambda ((syms . fun))
   ;;    (mapcar (lambda (sym) `(void-add-hook ',sym #',fun))
...
(setq use-package-keywords (-remove-item :bind use-package-keywords))
(setq use-package-keywords (-remove-item :bind* use-package-keywords))
(setq use-package-keywords (-remove-item :bind-keymap use-package-keywords))
(setq use-package-keywords (-remove-item :bind-keymap& use-package-keywords))
#+end_src
