...
(defmacro use-package! (package &rest args)
  `(use-package ,package
     ,@(-replace-where #'keywordp (-partial #'alist-get ) args)))
#+end_src
...
One thing I miss about leaf is alias support.
...
In the `use-package!' macro, keywords will be repaced with DEF.")
#+end_src
...
**** thook
:PROPERTIES:
:ID:       a24fb94b-7217-411d-86c6-144e34044a01
:END:
...
**** hook                                                             :disabled:
:PROPERTIES:
:ID:       cd344df8-3345-4870-be25-df0003916ca8
:END:
...
#+begin_src emacs-lisp
(defadvice! abbrev-hooks-and-modes (:override use-package-normalize/vhook)
  (thread-last (use-package:args-or-error <args> (-orfn #'symbolp #'-cons-pair-p))
    (--map-when (not (-cons-pair-p it)) (cons it nil))
    (--map (cons (use-package:ensure-hook (car it))
                 (use-package:ensure-mode (cdr it) <name>)))))
...
***** handler
:PROPERTIES:
:ID:       f5148b34-4d64-4dbd-af0f-2d5429e16f37
:END:

#+begin_src emacs-lisp
(defadvice! use-void-add-hook (:override use-package-handler/vhook)
  (use-package-concat
   (use-package-process-keywords <name> <rest> <state>)
   (mapcar (-lambda ((hook . fn)) `(void-add-hook ',hook #',fn))
           (use-package-normalize-commands <args>))))
...
(void-add-advice #'use-package-normalize/:bind :override #'ignore)
(void-add-advice #'use-package-handler/:bind :override #'ignore)
#+end_src
