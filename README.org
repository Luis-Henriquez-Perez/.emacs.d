...
#+begin_src emacs-lisp
(defun leaf~insert-after (place pair plist)
  "Insert KEY and VALUE before PLACE in `leaf-keywords'."
  (alet (+ 2 (-elem-index place leaf-keywords))
    (->> leaf-keywords
         (-insert-at it value)
         (-insert-at it key))))
(defun leaf~insert-after (place key value plist)
  "Return PLIST with KEY and VALUE inserted before PLACE."
  (-let ((index (+ 2 (-elem-index place plist))))
    (thread-last plist
      (-insert-at index value)
      (-insert-at index key))))#+end_src

**** remove elements
:PROPERTIES:
:ID:       3e941c3e-23c8-4f5d-bc53-467e87398cac
:END:

#+begin_src emacs-lisp
(defun leaf~remove-existing (key)
  (aif (->> (--find-indices (eq it key) leaf-keywords)
            (-annotate #'1+)
            (flatten-list))
      (-remove-at-indices it leaf-keywords)
    leaf-keywords))
#+end_src
...
[[id:1e0f3a27-a7d8-4e28-a359-f42ed7a16033][defadvice!]] and [[id:4daf2baf-ea7f-41f5-9f86-63168089149a][defhook!]], the syntax is designed to resemble [[helpfn:defun][defun]]. The
motivation for creating this is that while.
...
(defmacro defleaf! (name args &rest body)
  ...
  (-let* ((wrap-p (s-suffix-p "*" (symbol-name name)))
          ...
          (leaf-insert (void-symbol-intern 'leaf~insert- where))
          (leaf-apply (void-symbol-intern 'leaf~~apply- name)))
    `(progn
       (setq leaf-keywords (leaf~remove-existing ,keyword))
       (setq leaf-keywords
             (funcall #',leaf-insert
