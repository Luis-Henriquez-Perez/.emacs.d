...
*** useful aliases
:PROPERTIES:
:ID:       e15e3361-32a5-460c-9da5-10512c7a89ff
:END:
...
*** bootstrap
:PROPERTIES:
:ID:       74028d3c-9500-4b2c-8425-a36dd6979de4
:END:
...
***** another normalizer
:PROPERTIES:
:ID:       c7fd7beb-b85a-4f60-abeb-794ed33988f6
:END:
...
are in [[helpvar:leaf-keywords][leaf-keywords]]. Therefore, it's important to be able to add keywordsThe purpose of these functions is to make it extremely easy to add key value
pairs to the plist in the right location.
...
#+begin_src emacs-lisp
(defun leaf~insert-before (plist place key value)
  (leaf-insert-list-before plist place (list key value)))
#+end_src
...
#+begin_src emacs-lisp
(defun leaf~insert-after (plist place key value)
  "Insert TARGETLIST."
  (alet (it (+ 2 (-elem-index place plist)))
    (->> plist
         (-insert-at it key)
         (-insert-at (1+ it) value))))
#+end_src

**** insert at
:PROPERTIES:
:ID:       c87aaf35-af3e-41d1-af07-07e77742ec97
:END:

#+begin_src emacs-lisp
(defun leaf~insert-at (plist place key value)
  "Insert TARGETLST at ALEM in LIST."
  (let ((i (-elem-index elem list))
        (j (+ i 1)))
    (->> (-replace-at i key plist)
         (-replace-at r value plist))))
#+end_src
...
This function handle inserting a leaf keyword.

#+begin_src emacs-lisp
(defun leaf~insert-keyword (where place key value)
  "Insert KEYWORD at PLACE.
WHERE can be the symbol `before', `after' or `at'. When WHERE is before KEYWORD
is inserted before PLACE. When WHERE is `after', KEYWORD is inserted after PLACE."
  (cl-assert (member where '(at before after)))

  (alet (void-symbol-intern 'leaf~insert- where)
    (setq leaf-keywords (funcall it leaf-keywords key value))))
#+end_src
...
:PROPERTIES:
:ID:       fd75a615-9a74-4cbb-9ce9-7a9df3462066
:END:

#+begin_src emacs-lisp
(defun leaf:flatten-keywords (keywords)
  "Flatten keywords."
  (->> (-partition-before-pred #'keywordp args)
       (--map (-flatten-n 1 it))
       (--map (-annotate (-const (car it)) (cdr it)))))
#+end_src
...
#+begin_src emacs-lisp
(defvar leaf~process-variables '(raw name key keyname value body)
...
This macro is meant to make defining a leaf keyword easy.

#+begin_src emacs-lisp
(defmacro defleaf! (name args &rest body)
  "Define a leaf keyword."
  (declare (indent defun))
  (-let* (((where place alias defer normalizer) args)
          (keyword (void-keyword-intern name))
          (leaf-vars leaf~process-variables)
          (fname (void-symbol-intern 'leaf~apply- name))
          (vars (-map (-partial #'void-symbol-intern 'leaf--) leaf-vars))
          (anaphoric-vars (-map #'void-anaphoric-symbol leaf-vars)))
    `(progn
       (leaf~insert-keyword )

       ,(alet `(,fname ,@vars)
          `(leaf~insert-keyword ,keyword ',where ,place ',it))

       ,@(--map `(leaf~set-alias ,keyword ,it) (-list alias))

       ,@(whenlist! defer `(leaf~set-defer ,keyword))

       ,@(whenlist! normalizer `(leaf~set-normalizer ,keyword ,normalizer))

       (defun ,fname ,anaphoric-vars
         (-let [,vars ,anaphoric-vars]
           (ignore ,@vars ,@anaphoric-vars)
           ,@body)))))
#+end_src
...
#+begin_src emacs-lisp
(defleaf! defer (before leaf-defer :leaf-defer)
  (if (and <body> (eval (car <value>))
           (leaf-list-memq leaf-defer-keywords (leaf-plist-keys <raw>)))
      `((after! ,<name> ,@<body>))
    `(,@<body>)))
#+end_src
...
#+begin_src emacs-lisp
(defleaf! before-call (before :hook nil t commands)
  (let ((advice (void-symbol-intern 'load- )))
    `(progn (void-advice-add ,<value> :before )
            ;; (defadvice! ,advice (before)
            ;;   "Load package."
            ;;   (require ',))
            )))
#+end_src
...
#+begin_src emacs-lisp
(defleaf! after-call (before :hook nil t commands)
  (let ((advice (void-symbol-intern 'load- )))
    `(progn (void-advice-add ,<value> :before )
            (defadvice! ,advice (after)
              "Load package."
              (require ',)))))
#+end_src
...
#+begin_src emacs-lisp
(defleaf! idle-require (before before-call nil t)
  `(idle-require ,<value>))
#+end_src
...
#+begin_src emacs-lisp
(defleaf! void-straight (replace straight ensure straight)
  `(straight:install-package ,<value>))
#+end_src
...
#+begin_src emacs-lisp
(defleaf! popup (before :mode)
  `(push ',it display-buffer-alist) ,<value>)
#+end_src
...
#+begin_src emacs-lisp
(defleaf! os* (before :when)
  (when <body>
    `((with-os! ,leaf--value ,@leaf--body))))
#+end_src
