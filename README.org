...
This is my own simple macro that does essentially the same thing as leaf or
use-package but has far less lines of code and is much simpler. Some notable
differences are that I use [[id:2c2e4202-4530-4538-a8c9-ffb40239d6042c2e4202-4530-4538-a8c9-ffb40239d604][weighted sorting]] (similar to [[helpfn:add-hook][depth]] in hooks added
after Emacs 26) instead of explicit; I use alists instead of plists. The goal of
=def-package= is to make adding new keywords as easy and straightforward as
possible (see [[8c42eac6-a0ae-448a-af07-100d0c6a06ac][def-package-key]] and [[id:7f249971-adf4-488e-a18f-cbcb5937a7c4][custom keywords]]). I also want the data
structures and the mechanisms behind generating a form to be easy to test, read,
and hack.

*** keywords
:PROPERTIES:
:ID:       dc5dc74a-cd2f-4682-a76d-fe90b27c4fa1
:END:

Both leaf and use-package have their keywords as a plist. Instead, I use an
alist because alists are easier to manipulate and traverse programmatically than
plists (at least in my experience). And because alists structurally reflect the
fact that keys and values are related.

#+begin_src emacs-lisp
(defvar def-package-keywords nil
  "An alist whose elements are of the form (KEY . APPLY-FN).
Key is a key for a keyword. APPLY-FN is a function that accepts two argments.")
#+end_src

*** default keywords
:PROPERTIES:
:ID:       168b8c06-703b-4b51-9df5-d9d3d729288e
:END:

It is useful to apply some keywords to all emacs packages. Perhaps the most
common example of this is the =:ensure= keyword. "ensuring" refers to making sure
that a package is installed. Typically, its something you want to do to all your
packages to avoid getting errors when you try to use them.

#+begin_src emacs-lisp
(defvar def-package-default-keyword-alist nil
  "Keywords that should always be considered in `defpackage' forms.")
#+end_src

*** aliases
:PROPERTIES:
:ID:       828b4cf6-8323-4c17-b265-1ad598693ac3
:END:

Sometimes it can be useful to give a alias a key with another. Particularly when
we do not want to overwrite an existing key. One example of this is when you're
using a package manager other than =package.el=. Suppose you have a =straight=
keyword and you want to use it instead.

**** aliases
:PROPERTIES:
:ID:       c1c2e9e0-3e3c-4f9c-84e3-0a57bf5ec57e
:END:

#+begin_src emacs-lisp
(defvar def-package-alias-alist nil
  "An alist whose keys are (ALIAS . KEY).
ALIAS is the. KEY is.")
#+end_src

**** true key
:PROPERTIES:
:ID:       2aa8a4da-e14e-47e7-9c1d-bd0f3073e951
:END:


#+begin_src emacs-lisp
(defun def-package-true-key (key)
  "Return the true alias of KEY.
This function recurses infinitely if key has a circular alias."
  (aif (alist-get key def-package-alias-alist)
      (or (def-package-true-key it) it)
    key))
#+end_src

**** set alias
:PROPERTIES:
:ID:       059f8edf-5104-4a88-8251-7e1a98d9759c
:END:

#+begin_src emacs-lisp
(defun def-package-set-alias (alias original)
  "Alias ORIGINAL to ALIAS."
  (setf (alist-get alias def-package-alias-alist) original))
#+end_src

*** weighted sorting
:PROPERTIES:
:ID:       2c2e4202-4530-4538-a8c9-ffb40239d604
:END:

Instead of having a fixed sort order for keys like =use-package= and =leaf=,
=defpackage= gives each key a weight (similar to hooks have a depth after emacs
26). A weight is a number assigned to a key. Keys with a higher number will be
sorted closer to the beginning of the list than keys with a lower number. The
reasoning behind this is that typically for many keywords the order does not
matter--at least as long as they're after a certain set of other keywords. For
example.

**** weight
:PROPERTIES:
:ID:       08378608-d794-4923-8f1c-20a09066af18
:END:

#+begin_src emacs-lisp
(defvar def-package-weight-alist nil
  "An alist of the form (KEY . WEIGHT).
KEY is a `def-package' key. WEIGHT is the number used to sort the key")
#+end_src

**** default weight
:PROPERTIES:
:ID:       01cac514-3dfa-4c72-82d3-b9df7f3db9cc
:END:

#+begin_src emacs-lisp
(defvar def-package-default-weight 0
  "Default weight for a `def-package' keyword.")
#+end_src

**** getter for weight
:PROPERTIES:
:ID:       a1df5773-2251-4b90-a65d-43f8c23ba4d8
:END:

#+begin_src emacs-lisp
(defun def-package-weight (key)
  "Return the weight for the given key."
  (alist-get key def-package-weight-alist))
#+end_src

**** setter for weight
:PROPERTIES:
:ID:       310e44c4-3629-4537-97ad-4cb87474b3d8
:END:

#+begin_src emacs-lisp
(defun def-package-set-weight (key weight)
  "Set the KEY to WEIGHT."
  (setf (alist-get key def-package-weight-alist) weight))
#+end_src

**** sort keys by weight
:PROPERTIES:
:ID:       a28beb1a-f721-4d74-8d9b-a9a6374249c0
:END:

#+begin_src emacs-lisp
(defun def-package-weight-sorter (pair1 pair2)
  "Return non-nil if PAIR1 should be sorted before PAIR2"
  (> (def-package-weight (car pair1))
     (def-package-weight (car pair2))))
#+end_src

*** normalizing
:PROPERTIES:
:ID:       7c0cfad3-7913-4d25-a69d-ac1ac9219c29
:END:

Each key can take values of a different form. For example, the [[id:60af5e51-ca3b-4776-8cf7-d58f223180d3][hook keyword]] can
take a cons cell of the form ~(hook . value)~, or a just the hook variable, or a
list of these. Functions created for customizing a keyword are easier to write
and easier to read if they just assume one standard structure for their value.
Thinking back on the hook keyword, each individual value can be thought of as a
cons cell. Normalizers allow us to separate. The reason that the normalizers
take a list instead of a value (and then use that to map through the list) is to
deal with nested lists.

**** normalizer alist
:PROPERTIES:
:ID:       69e9fbe3-d04d-45d6-a3d6-82ccaaca40a4
:END:

#+begin_src emacs-lisp
(defvar def-package-normalizer-alist nil
  "An alist of (KEY . NORMALIZER).
NORMALIZER is with one argument, the list of values for keyword KEY. It should
return a normalized list of values..")
#+end_src

**** set a normalizer
:PROPERTIES:
:ID:       69f983aa-1009-424a-adf2-c9c80947b42b
:END:

#+begin_src emacs-lisp
(defun def-package-set-normalizer (key normalizer)
  "Add (KEY . NORMALIZER) to `def-package-normalizer-alist'."
  (setf (alist-get key def-package-normalizer-alist) normalizer))
#+end_src

**** expand values
:PROPERTIES:
:ID:       1e44ea35-23c2-4143-8b68-1ac33e6c80a6
:END:

A key theme of =defpackage= implementation is for user-defined key functions only
having to deal with one value as opposed to the list of values. The point of
expanding the values is to make it so that each apply function can be applied to
every element in the list.

#+begin_src emacs-lisp
(defun def-package-expand-values (pair)
  "Expand values pair."
  (-let (((key . values) pair))
    (-annotate (-const key) values)))
#+end_src

**** apply normalization to a pair
:PROPERTIES:
:ID:       b1c9696f-67a9-459b-9199-85dcc6fa96c1
:END:

This function is what applies the normalization to each of the values.

#+begin_src emacs-lisp
(defun def-package-apply-normalize (pair)
  "Apply normalizations from `def-package-normalizer-alist' to values."
  (-let [(key . values) pair]
    (aif (alist-get key def-package-normalizer-alist)
        (cons key (funcall it values))
      pair)))
#+end_src

**** normalize list pairs
:PROPERTIES:
:ID:       6331c2b7-c5e8-43d2-9bc1-05e6771ad613
:END:

Normalizing consists of four steps. First keys and their corresponding values
are separated from one another. Then they are sorted by the weight of the keys.
Then, values are normalized. And finally, values are expanded.

#+begin_src emacs-lisp
(defun def-package-normalize-args (args)
  "Convert args into an alist such that (KEY . VALUE)."
  (->> (-partition-by-header #'keywordp args)
       (-sort #'def-package-weight-sorter)
       (mapcar #'def-package-apply-normalize)
       (mapcan #'def-package-expand-values)))
#+end_src

**** normalizers
:PROPERTIES:
:ID:       7009f29c-884e-4f3d-8a86-eb05b9ae2154
:END:

#+begin_src emacs-lisp
;; For a value that tkes a list of forms.
(defun def-package-ensure-form-list (values)
  "Ensure that"
  (list values))

(defun def-package-ensure-cons-cells (values)
  "Ensure that values are all cons cells."
  (--splice (> (safe-length it) 1) (def-package-ensure-cons-cells it)
            (mapcar #'-list values)))
#+end_src

*** def-package macro
:PROPERTIES:
:ID:       b265f9e3-c50f-42c2-a29c-f734f168ea99
:END:

The gist is this. What we want to do is convert the plist arguments into an
alist mapping keys to exactly one input. Then we "apply" each key and value to
the form we want to produce (initially nil) to get our desired form.

**** main apply function
:PROPERTIES:
:ID:       9d32f14a-61d0-4200-8797-4a7ed776f5f8
:END:

This function is meant to be used with [[helpfn:-reduce][-reduce]] to construct the body of the form
constructed by the =def-package= macro. Often there is some metadata that is
useful to know when executing these functions such as the package name (see
[[][my definition of :hook]). This metadata will be included in BINDINGS.

#+begin_src emacs-lisp
(defun def-package-apply-fn (pair forms &optional bindings)
  "Return new forms from applying PAIR to FORMS.

\((VAR . VALUE) &optional BINDINGS)"
  (-let* (((key . value) pair)
          (apply-fn (alist-get (def-package-true-key key) def-package-keywords)))
    (if (null apply-fn)
        (error "defpackage - function for %s not defined "key)
      (funcall apply-fn value forms bindings))))
#+end_src

**** convenience apply
:PROPERTIES:
:ID:       91e5119a-2606-4703-a964-8596e2fb7708
:END:

#+begin_src emacs-lisp
(defun def-package-apply (key value &optional bindings)
  "A convenience apply function."
  (def-package-apply-fn (cons key value) nil bindings))
#+end_src

**** produce-form
:PROPERTIES:
:ID:       b91919e5-de59-44f2-b39a-6ce737a906f2
:END:

#+begin_src emacs-lisp
(defun def-package-forms (alist &optional bindings)
  "Return list of forms generated by ALIST.
BINDINGS are an alist of (VAR . VALUE)."
  (--reduce-from (def-package-apply-fn it acc bindings) nil (reverse alist)))
#+end_src

**** def-package
:PROPERTIES:
:ID:       5e5e10db-e42b-4d03-b986-d10524f1017f
:END:

#+begin_src emacs-lisp
(defmacro def-package (package &rest args)
  "A replacement for `use-package' and `leaf'."
  `(progn
     ,@(def-package-forms (def-package-normalize-args args)
         `((<package> . ,package)))))
#+end_src

*** alias for built in
:PROPERTIES:
:ID:       7d617f47-8bba-4380-b1c2-35d9b658500f
:END:

#+begin_src emacs-lisp
(defalias 'def-feature! 'def-package)
(defalias 'def-package! 'def-package)
#+end_src

*** custom keywords
:PROPERTIES:
:ID:       e73b7f95-1082-4978-a501-f69f6d516cbb
:END:

Defining a keyword is as simple as writing a function that returns the form you
want in the resulting =def-package= form and adding it to to the [[helpfn:][]].

**** define a package key
:PROPERTIES:
:ID:       8c42eac6-a0ae-448a-af07-100d0c6a06ac
:END:

This is a convenience macro used for defining new keywords. The whole point of
it is to make adding new keys as easy as possible.

#+begin_src emacs-lisp
(defmacro def-package-key (name args &rest body)
  "Define a `def-package' keyword named NAME.
ARGS is a list of two arguments."
  (declare (indent defun))
  (-let ((key (void-keyword-intern name))
         (fn (void-symbol-intern 'def-package--apply- name))
         (value (make-symbol "value"))
         (forms (make-symbol "forms"))
         (bindings (make-symbol "bindings")))
    `(progn
       (setf (map-elt def-package-keywords ,key) #',fn)
       (setf (alist-get ,key def-package-weight-alist)
             def-package-default-weight)
       (defun ,fn (,value ,forms ,bindings)
         (cl-progv (map-keys ,bindings) (map-values ,bindings)
           (-let [,args (list ,value ,forms)]
             ,(if (= 1 (length args))
...
**** custom keywords
:PROPERTIES:
:ID:       7f249971-adf4-488e-a18f-cbcb5937a7c4
:END:

This headline contains my long list of custom keywords. Most are based on those
from =use-package= and =leaf=.

***** wrappers
:PROPERTIES:
:ID:       7a61245e-fc90-4b11-9108-530b5067fdea
:END:

****** after
:PROPERTIES:
:ID:       e1f74cf5-fe9d-414c-8a8a-64f012345f7f
:END:

#+begin_src emacs-lisp
(def-package-key after (features body)
  `(after! ,features ,@body))

(def-package-set-weight :after 20)

(def-package-set-normalizer :after #'def-package-ensure-form-list)
#+end_src

****** os
:PROPERTIES:
:ID:       e49b9086-a18e-4062-996b-0a0c77fb7447
:END:

#+begin_src emacs-lisp
(def-package-key os (os body)
  `(with-os! ,os ,@body))

(def-package-set-weight :os 20)
#+end_src

****** when
:PROPERTIES:
:ID:       6c69b275-2381-4a3d-af3f-ae4fe1f79be5
:END:

#+begin_src emacs-lisp
(def-package-key when (condition body)
  `(when ,condition ,@body))

(def-package-set-weight :when 20)
#+end_src

****** disable
:PROPERTIES:
:ID:       755e4c53-1a9a-4c7a-bdca-3e6e600c35b1
:END:

#+begin_src emacs-lisp
(def-package-key disabled (bool body)
  (if bool nil body))

(def-package-set-weight :disable 25)
#+end_src

***** package management keywords
:PROPERTIES:
:ID:       d33d57d5-52d5-4f08-aea3-046edcae61a7
:END:

****** straight
:PROPERTIES:
:ID:       094f3c30-4dd9-426e-8e6d-54e80d672023
:END:

#+begin_src emacs-lisp
(def-package-key straight (recipe)
  `(progn
     (unless (featurep 'straight) (straight:init-fn))
     (condition-case err
         (progn (straight-use-package ,recipe)
...
****** package.el
:PROPERTIES:
:ID:       e850e501-182d-4a99-9442-bab18c55934b
:END:

#+begin_src emacs-lisp
(def-package-key package (recipe)
  )
#+end_src

***** setting values
:PROPERTIES:
:ID:       cc2442a3-766f-4b42-ae80-e618f140eac1
:END:

****** pre-setq
:PROPERTIES:
:ID:       e9a238b4-884b-421d-bd89-d017bd818944
:END:

#+begin_src emacs-lisp
(def-package-key pre-setq ((var . value))
  `(setq ,var ,val))

(def-package-set-weight :pre-setq 15)
#+end_src

****** setq
:PROPERTIES:
:ID:       ab0115b9-8384-4a5e-8db5-754f2e0cdcc4
:END:

#+begin_src emacs-lisp
(def-package-key setq (pairs)
  `(after! ,<package>
     ,@(-map (-lambda ((var . val)) `(setq ,var ,val)) pairs)))

(def-package-set-normalizer :setq #'def-package-ensure-form-list)
#+end_src

****** setq-default
:PROPERTIES:
:ID:       33b9ec4c-d74d-40ed-bc4e-cf823f80b7ec
:END:

#+begin_src emacs-lisp
(def-package-key setq-default ((key . value))
  `(setq-default ,key ,value))
#+end_src

****** custom
:PROPERTIES:
:ID:       dfd32d6f-0175-4fdd-9300-a6541fa1df24
:END:

#+begin_src emacs-lisp
(def-package-key custom ((var . value))
  `(customize-set-variable ',var ,value))
#+end_src

***** deferred loading
:PROPERTIES:
:ID:       b68a9a98-d584-4b6a-a936-005e7faeb6dc
:END:

****** require
:PROPERTIES:
:ID:       36959112-ec34-467d-b46f-425cd5d06083
:END:

#+begin_src emacs-lisp
(def-package-key require (package)
  (when package `(require ',<package>)))
#+end_src

****** command
:PROPERTIES:
:ID:       a30f2868-1498-451d-85b3-60469e7dc022
:END:

I choose to name this "command" instead of "commands" to be consistent with how
other keywords such as "hook" are named.

#+begin_src emacs-lisp
(def-package-key command (command)
  `(unless (fboundp ',package)
     (autoload #',value ,(symbol-name package) nil t)))
#+end_src

****** before-call
:PROPERTIES:
:ID:       4899152a-ff5f-4301-a839-2e873a407266
:END:

#+begin_src emacs-lisp
(def-package-key before-call (fn)
...
****** after-call
:PROPERTIES:
:ID:       adc31b5a-76ed-42b8-835a-1fef901d3fdc
:END:

#+begin_src emacs-lisp
(def-package-key after-call (fn)
  `(defadvice! ,(void-symbol-intern 'load- <package>) (:before ,fn)
     (require ',<package>)))
#+end_src
...
****** defer-config
:PROPERTIES:
:ID:       59c4bdad-ab90-432a-89e7-1ab901edac84
:END:

#+begin_src emacs-lisp
(def-package-key defer-config (body)
  (after! ,<package> ,@body))
#+end_src

****** hook
:PROPERTIES:
:ID:       60af5e51-ca3b-4776-8cf7-d58f223180d3
:END:

#+begin_src emacs-lisp
(def-package-key hook ((hook . fn))
  (alet (or fn (void-symbol-intern <package> '-mode))
    `(progn (void-add-hook ',hook #',it)
            ,@(def-package-apply :command it))))

(push (cons :hook #'def-package-ensure-cons-cells) def-package-normalizer-alist)
#+end_src

***** miscellaneous
:PROPERTIES:
:ID:       87bf82fd-5c71-43a5-8a8f-67f42697ff3d
:END:

****** init
:PROPERTIES:
:ID:       a3273c87-ba59-4ed7-a08b-3bf6d80bca53
:END:

#+begin_src emacs-lisp
(def-package-key init (form) form)
#+end_src

****** mode
:PROPERTIES:
:ID:       e4126fe1-3515-4d21-b178-137094d00ecb
:END:

#+begin_src emacs-lisp
(def-package-key mode ((regexp . mode))
  `(add-to-list 'auto-mode-alist (cons ,regexp ',mode)))
#+end_src

****** config
:PROPERTIES:
:ID:       09342213-5059-45eb-966f-255b3b2848ee
:END:

#+begin_src emacs-lisp
(def-package-key config (form) form)
#+end_src

****** load-path
:PROPERTIES:
:ID:       412683d3-325f-477e-9927-b9c0a7480481
:END:

#+begin_src emacs-lisp
(def-package-key load-path (path)
  `(add-to-list 'load-path ,path))
#+end_src

****** alias
:PROPERTIES:
:ID:       49135d18-70c6-4f5e-98ce-c01a8ecaf13b
:END:

#+begin_src emacs-lisp
(def-package-key alias ((alias . definition))
  `(eval-and-compile (defalias #',alias #',definition)))

(def-package-set-normalizer :alias #'def-package-ensure-cons-cells)
#+end_src

****** bind
:PROPERTIES:
:ID:       ed2e5151-003b-40cb-b3b3-9011de8716c7
:END:

#+begin_src emacs-lisp
(def-package-key bind ()
  `(general-def ))
#+end_src
...
#+begin_src emacs-lisp
(def-package dash-functional :require t)
#+end_src
...
(def-package shut-up
  :command shut-up
  :alias ((shut-up! . shut-up) (quiet! . shut-up)))
#+end_src
...
#+begin_src emacs-lisp
(def-package keyfreq :hook emacs-startup-hook)
#+end_src
...
***** TODO exec path from shell                                       :disable:
:PROPERTIES:
...
environment, so envars will be wrong. That includes the path
...
(def-package ace-window
  :command ace-window ace-swap-window
...
(def-package eyebrowse
  :command eyebrowse-create-window-config
...
(def-package transpose-frame
  :command transpose-frame flip-frame flop-frame
  :command rotate-frame rotate-frame-clockwise)
#+end_src
...
#+begin_src emacs-lisp
(def-package exwm-edit :after exwm)
#+end_src
...
#+begin_src emacs-lisp
(def-package counsel-org-capture-string
  :after org ivy
  :command counsel-org-capture-string)
#+end_src
...
#+begin_src emacs-lisp
(def-package counsel-web
  :command counsel-web-suggest counsel-web-search
...
#+begin_src emacs-lisp
(def-package company-prescient :hook company-mode-hook)
#+end_src
...
*** going to url                                                      :disable:
:PROPERTIES:
:ID: 1079a7c8-e596-40d4-b86e-e916944e9030
:END:

#+begin_src emacs-lisp
(def-package xwwp-follow-link
...
#+begin_src emacs-lisp
(def-package screenshot :command screenshot)
#+end_src
...
#+begin_src emacs-lisp
(def-package scrot :ensure (:host github :repo "dakra/scrot.el"))
#+end_src
...
** highlight-escape-sequences                                         :disable:
:PROPERTIES:
...
(def-package hideshow
  ...
  :command hs-minor-mode hs-toggle-hiding hs-already-hidden-p
...
#+begin_src emacs-lisp
(def-package org-link-minor-mode
  :command org-link-minor-mode)
#+end_src
...
#+begin_src emacs-lisp
(def-package aggressive-indent :hook emacs-lisp-mode-hook)
#+end_src
...
#+begin_src emacs-lisp
(def-package ialign :command ialign)
#+end_src
...
(def-package lispyville
  :command lispyville-comment-or-uncomment-line
...
#+begin_src emacs-lisp
(def-package aggressive-fill-paragraph)
#+end_src
...
#+begin_src emacs-lisp
(def-package define-it
  :command define-it define-it-at-point
  :setq
...
**** package specific setup                                           :disable:
:PROPERTIES:
...
#+begin_src emacs-lisp
(def-package evil-surround :hook prog-mode-hook text-mode-hook)
#+end_src
...
#+begin_src emacs-lisp
(def-package evil-matchit :hook prog-mode)
#+end_src
...
(def-package expand-region
  :command er/expand-region er/contract-region er/mark-symbol er/mark-word
  :init
  (general-def 'visual
...
(def-package eshell-up
  :after eshell
  :command eshell-up eshell-up-peek
  :init
...
(def-package command-log-mode
  :command command-log-mode global-command-log-mode
  :setq
...
#+begin_src emacs-lisp
(def-package all-the-icons-dired :hook ranger-mode-hook)
#+end_src
...
#+begin_src emacs-lisp
(def-package restart-emacs :command restart-emacs)
#+end_src
...
#+begin_src emacs-lisp
(def-package git-gutter :command git-gutter-mode)
#+end_src
...
** TODO autorevert                                                    :disable:
:PROPERTIES:
:ID: 1d7c8bac-e523-4251-bb3c-b85f592fbfc3
:END:

#+begin_src emacs-lisp
(def-feature! autorevert
...
#+begin_src emacs-lisp
(def-package password-generator
  :command password-generator-simple password-generator-paranoid)
#+end_src
...
(def-package persistent-scratch
  :command persistent-scratch-restore
...
#+begin_src emacs-lisp
(def-package sudo-edit :command sudo-edit)
#+end_src
...
#+begin_src emacs-lisp
(def-package cider :command cider)
#+end_src
...
(def-package outorg
  ...
  :command outorg-convert-back-to-code outorg-convert-to-org)
...
(def-package macrostep
  :command (macrostep-expand macrostep-collapse macrostep-collapse-all)
...
#+begin_src emacs-lisp
(def-package markdown-mode :mode "\\.md\\'")
#+end_src
...
#+begin_src emacs-lisp
(def-package hide-lines
  :command hide-lines hide-lines-matching)
#+end_src
...
#+begin_src emacs-lisp
(def-feature! org-agenda
  :after org
  :command (org-agenda org-agenda-list)
  :setq
...
#+begin_src emacs-lisp
(def-feature! org-clock
  :command org-clock-in
...
#+begin_src emacs-lisp
(def-feature! org-crypt
  :command org-encrypt-entries org-encrypt-entry org-decrypt-entries org-decrypt-entry
...
**** narrowing and widening                                           :disable:
:PROPERTIES:
...
*** install all the icons if not installed                            :disable:
:PROPERTIES:
