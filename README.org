...
(defun use-package-normalize/:setq (name keyword args)
  (aprog1 (use-package:args-as-one args)
    (unless (-all-p #'consp it)
      (thread-last (-find (-not #'consp) it)
        (format "All arguments should be cons cells. %S isn't though.")
        (use-package-error)))))
...
(defun use-package-handler/:popup (name _keyword args rest state)
  (mapcar `(push ',it display-buffer-alist) args))
#+end_src
