   ...
   (list
    (when-let ((commands (plist-get rest :commands))
      ...
      `(defadvice! ,advice (:around ,@commands)
         ...
         (if-let (uninstalled (-remove #'system-packages-package-installed-p ',packages))
             (aif (y-or-n-p ,(format "%S is needed for %s to work.\nWould you like to install it?" packages name))
                 (progn (-map #'system-packages-install uninstalled)
                        (apply <orig-fn> <args>)))
           (apply <orig-fn> <args>)))))))
#+end_src
