...
:END:
  ...
  ensuring that at a glance any reader knows the precise order in which files
  are evaluated.  Moreover, I divide these files into "init" files that are
  evaluated immediately, and "config" files which are evaluated after some
