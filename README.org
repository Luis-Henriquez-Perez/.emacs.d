...
#+begin_src emacs-lisp
(defadvice! use-my-custom-normalize-fn (:override use-package-normalize/:hook)
  (aprog1 (thread-last "a symbol or cons cell"
            (use-package:args-or-error <keyword> <args> (-orfn #'symbolp #'consp))
...
#+begin_src emacs-lisp
(defadvice! use-my-custom-handler-fn (:override use-package-handler/:hook)
  (use-package-concat
   (use-package-process-keywords <name> <rest> <state>)
...
****** set an alias                                                   :disabled:
:PROPERTIES:
:ID:       4804d722-05ed-4da3-a448-9557a2416465
:END:

#+begin_src emacs-lisp
(push (cons :hook :vhook) use-package:aliases)
#+end_src
