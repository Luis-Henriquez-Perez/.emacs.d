...
A key theme of =defpackage= implementation is to allow. The point of expanding the
values is to make it so that each apply function can be applied to every element
in the list.
