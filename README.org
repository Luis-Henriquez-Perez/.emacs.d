...
(defun use-package-normalize/:setq (name keyword args)
  (use-package:unfold-enlisted args (lambda (it) (and (consp it) (symbol-p (car it))))))
...
(defun use-package:args-or-error (args valid-p)
  (-let (((args invalid-elt) (use-package:unfold-args args)))
    (if invalid-elt
        (use-package-error "Invalid element: %S" elt)
      args)))
#+end_src
