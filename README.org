...
Package configuration can be a monotonous process. In this headline, I configure
=use-package=, a declaration macro designed to provide a concise syntax for
configuring packages in a performance-oriented way.
...
built-in packages and external packages. The idea was taken from [[][]]. The
only difference is that with use-feature! ensure is always nil.
...
This function strives to be more concise than [[][]]. Also, unlike the latter,
...
In this headline I define custom use-package keywords.
...
This keyword provides integration of my [[][]] with =use-package=. is for installing
packages based on my operating system.
...
These keywords are based on keywords in [[][leaf]], an alternative to
=use-package=. They distinguish setting variables from the rest of the configuration.
...
=:before-call= and its counterpart =:after-call= let me put off package loading
