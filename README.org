...
**** defhook-target!
:PROPERTIES:
:ID:       4b08a38a-2a7e-42c2-81f5-f6be600ab271
:END:

The need arises to define hooks. The inspiration for this macro came from
[[https://github.com/hlissner/doom-emacs/blob/develop/core/core-lib.el#L490][doom-emacs]].

#+begin_src emacs-lisp
(defmacro defhookvar! (name triggers &rest body)
  "Define a hook variable."
  (let ((hooks (-filter #'void:hook-p triggers))
        (advices (-remove #'void:hook-p triggers)))
    `(progn (defvar ,name ,docstring)
            (put ,name 'permanent-local t)
            (defhook! run-<name> (,@hooks)
              (run-hook <hook>))
            (defadvice! run-<name> (:before ,@advices)
              (run-hooks <hook-name>)))))
#+end_src
...
(defadvice! play-sound (:override play-pomodoro-sound)
...
#+begin_src emacs-lisp
(defadvice! disable-output (:around ranger-window-check)
