...
(defun! oo-startup-time-table ()
  ...
  (flet! add-percentage (time) (round (* 100 (oo-float-divide (cl-second time) total))))
  (flet! add-init-per (time) (round (* 100 (oo-float-divide (cl-second time) (string-to-number (emacs-init-time "%.2f"))))))
